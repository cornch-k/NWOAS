...
def configure_tahoe_display():
    ...
    assert len(timings) == 4 + struct.unpack_from("<I", timings)[0] * 24
    assert len(colors) == 4 + struct.unpack_from("<I", colors)[0] * 24
    advertised = [timings[4 + j * 24:28 + j * 24] for j in range(nt)]
    timing = next((x for fps in (60 << 16, 3928232) for x in advertised
                   if struct.unpack("<6I", x) == (1, 1280, 720, fps, 0, 0)), None)
    assert timing is not None, "No 720p 60/59.94 flag0 timing"
