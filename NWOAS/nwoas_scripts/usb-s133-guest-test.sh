#!/bin/bash
# S133: one owner for AP startup. PMGR requests are gated; PSCI starts APs.
set -eu
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s133-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[S133] MADT8 + PMGR AP gate + PSCI single-owner startup; PMCC cap32; S130 NVMe" | tee -a "$LOG"
