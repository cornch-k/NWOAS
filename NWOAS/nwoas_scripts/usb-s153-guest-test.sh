#!/bin/bash
# S153: S152 plus USB-C event-ring visibility and reset-safe IRQ gating.
set -eu
...
HV="$M1N1/build/m1n1-s153-usbc-event-coherence.bin"
PAYLOAD="$M1N1/m1n1-payload-s135-8cpu-speed.bin"
USBC="$M1N1/pcie_emul-d81-usbc-payload.py"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s153-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 916e402e45a101563f861f925d14da95836ad188b71dd04a6c7051a9feaf3180 ]
...
echo '[S153] S152 + USB-C event invalidate before IRQ + USBCMD.INTE gate' | tee -a "$LOG"
