#!/bin/bash
# S6-D10 continuation: power ACK received; host rejected valid empty setter response.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-pattern-resume-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
try:
    ...
    link=TahoeLink(i,p,(896,896,2816,2816),os.environ['NWOAS_PROTOCOL_LOG'])
    link.sequence,link.command_sequence=6,5
    u=ProxyUtils(p)
    ...
    print('[FACT] Resuming after successful power ACK; no repeat power command')
