#!/bin/bash
# S141: S140 CPU fix plus isolated 1 MiB direct ANS transfers.
set -eu
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s141-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo '[S141] S140 CPU P-states + MDTS/ANS direct transfer 64 KiB -> 1 MiB' | tee -a "$LOG"
