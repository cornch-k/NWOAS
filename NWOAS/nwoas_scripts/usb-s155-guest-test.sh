#!/bin/bash
# S155: S154 plus all-segment FL1100 event coherence.
set -eu
...
HV="$M1N1/build/m1n1-s155-fl1100-all-segments.bin"
PAYLOAD="$M1N1/m1n1-payload-s135-8cpu-speed.bin"
USBC="$M1N1/pcie_emul-d81-usbc-payload.py"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s155-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 7b5287872ddebd470517d5550c681ad6424c28c78262459145318998d5e5047a ]
...
echo '[S155] S154 + all-segment FL1100 event invalidate on EINT/IP' | tee -a "$LOG"
