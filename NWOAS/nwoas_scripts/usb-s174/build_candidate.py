#!/usr/bin/env python3
"""Build isolated S174 S172 RAM plus visible USB-C output; temporarily edit and exactly restore UEFI sources.
...
OUT=ROOT/'nwoas_scripts/usb-s174'
...
inner=inner.replace('OUT = ROOT / "nwoas_scripts/smp-s131"','OUT = ROOT / "nwoas_scripts/usb-s174/s131-build"')
inner=inner.replace('target = ROOT / "m1n1_windows/m1n1-payload-s131-8cpu.bin"',
                    'target = ROOT / "nwoas_scripts/usb-s174/m1n1-payload-s174-8cpu-usbc.bin"')
...
try:
    ...
    # S174 keeps original XHC1 visible; no other ACPI change.
    patched={p:p.read_bytes() for p in original}
    subprocess.run(['nice','-n','10','python3',str(OUT/'s131_builder.py')],cwd=ROOT,check=True)
    payload=OUT/'m1n1-payload-s174-8cpu-usbc.bin'
    manifest={'payload':str(payload),'bytes':payload.stat().st_size,
              'sha256':hashlib.sha256(payload.read_bytes()).hexdigest(),
              'change':'S161 normalization plus first 8 GiB high RAM kept; 8 CPUs; XHC1 visible, existing _CCA=1',
