...
root=Path('/Volumes/X31/NWOAS');repo=root/'apple_silicon_platforms_mu';out=root/'nwoas_scripts/uefi-s127'
...
original={p:p.read_bytes() for p in (hide,bm,inf)}
...
try:
 ...
 payload=old[:1376256]+fd;target=root/'m1n1_windows/m1n1-payload-s127-uefinvme-connect.bin';target.write_bytes(payload)
 manifest={'payload':str(target),'sha256':hashlib.sha256(payload).hexdigest(),'bytes':len(payload),'change':'S125 NVMe registration plus explicit BDS BeforeConsole recursive connect of NVMe handles only; original sources restored; hardware untested'}
