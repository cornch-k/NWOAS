#!/bin/bash
# S119 apply verified local SWMs after guarded cleanup of failed trial install.
# PASS: DISM exit0 and expected kernel/loader/hive; first boot remains untested.
# S119.TAG only; S100 must remain disarmed. S103/S102 binaries unchanged.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s119-$(date '+%Y%m%d-%H%M%S').XXXXXX")
