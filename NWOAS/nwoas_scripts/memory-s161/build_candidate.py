#!/usr/bin/env python3
"""Build isolated S161 output; temporarily edit and exactly restore UEFI sources.
...
OUT=ROOT/'nwoas_scripts/memory-s161'
...
inner=inner.replace('OUT = ROOT / "nwoas_scripts/smp-s131"','OUT = ROOT / "nwoas_scripts/memory-s161/s131-build"')
inner=inner.replace('target = ROOT / "m1n1_windows/m1n1-payload-s131-8cpu.bin"',
                    'target = ROOT / "nwoas_scripts/memory-s161/m1n1-payload-s161-8cpu-usba.bin"')
...
patched={}
try:
    subprocess.run(['git','apply','--check',str(OUT/'nwoas-s161-guest-ram-normalize.diff')],cwd=REPO,check=True)
    subprocess.run(['git','apply',str(OUT/'nwoas-s161-guest-ram-normalize.diff')],cwd=REPO,check=True)
    header.write_bytes((OUT/'Include/Library/NwoasGuestRam.h').read_bytes())
    ...
    payload=OUT/'m1n1-payload-s161-8cpu-usba.bin'
    manifest={'payload':str(payload),'bytes':payload.stat().st_size,
              'sha256':hashlib.sha256(payload.read_bytes()).hexdigest(),
              'change':'S102 backing normalization; 8 CPUs; XHC1 hidden; same Windows RAM cap',
    ...
    for p,b in original.items():
        if str(p) not in conflicts:
            p.write_bytes(b)
    if header.exists() and header.read_bytes()==(OUT/'Include/Library/NwoasGuestRam.h').read_bytes():
        header.unlink()
    if conflicts:
