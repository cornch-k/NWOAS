#!/bin/bash
# S203: S200 with source-rebuilt S197 compat guest m1n1 prefix.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/loader-s203-compat-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo '[S203] source-rebuilt S197 compat prefix; S200 control otherwise unchanged' | tee -a "$LOG"
