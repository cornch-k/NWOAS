...
R=Path('/Volumes/X31/NWOAS');repo=R/'m1n1_windows-s159';out=R/'nwoas_scripts/usb-s182'
...
for name in ['s175_dart1_translate.h','s175_dart1_translate.c']:
 p=repo/'src'/name;assert not p.exists();added[p]=(R/'nwoas_scripts/usb-s175'/name).read_bytes()
added[repo/'src/s182_crcr_latch.h']=(out/'crcr_latch.h').read_bytes()
...
# Capture both 32-bit halves and 64-bit writes before original MMIO write.
marker='                // NWOAS §6-C coherence: BEFORE'
assert s.count(marker)==1
s=s.replace(marker,'''                if ((elr >> 48) == 0xffff && (width == 2 || width == 3) &&
                    paddr >= S175_XHCI_BASE && paddr < S175_XHCI_BASE+0x10000) {
                    u64 op=S175_XHCI_BASE+(read32(S175_XHCI_BASE)&0xff);
                    if(paddr==op && width==2 && (val[0]&2))
                        nwoas_crcr_reset(&s182_crcr);
                    if(paddr==op+0x18 || paddr==op+0x1c)
                        nwoas_crcr_write(&s182_crcr,(unsigned)(paddr-op),1u<<width,
                                         val[0],!(read32(op)&1));
                }
'''+marker,1)
...
try:
 ...
 b=(repo/'build/m1n1.bin').read_bytes();target=out/'m1n1-s182-crcr-gap50.bin';target.write_bytes(b)
 manifest={'image':str(target),'bytes':len(b),'sha256':hashlib.sha256(b).hexdigest(),'change':'S182 CRCR write latch validation plus S163 gap50 plus USB-C DART1/SID1 translation preserving DAPF, no D83 rewrites or LOWALIAS; event seed remains0','source_sha256':{str(p.relative_to(repo)):hashlib.sha256(b).hexdigest() for p,b in {**patched,**added}.items()}}
