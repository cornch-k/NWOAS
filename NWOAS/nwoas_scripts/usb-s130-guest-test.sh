#!/bin/bash
# S130: S129 SSD-first boot with bounded NVMe diagnostic logging.
...
PAYLOAD="$M1N1/m1n1-payload-s129-ssd-first.bin"
HOOK="$M1N1/tahoe_dcp_guest_hook.py"
MODULE="$M1N1/pcie_emul-d81-usbc-payload.py"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s130-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = "f317a251a193c804a4ef6a0e7c142885c9365eebcc8200157083618b45f6273a" ]
if /usr/sbin/lsof -t "$DEV" >/dev/null 2>&1; then
    echo "STOP: serial busy"
    exit 1
fi
shasum -a 256 "$HV" "$PAYLOAD" "$HOOK" "$MODULE" >> "$LOG"
echo "[S130] S129 desktop baseline with bounded NVMe diagnostics" | tee -a "$LOG"
