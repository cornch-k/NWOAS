...
class Proxy:
 def __init__(self):self.calls=[];self.cap=0x5332323900000001;self.accept=1
 def nwoas_nvme_fastpath(self,*args):self.calls.append(args);return self.cap if args[0]==17 else self.accept
h=HV();p=Proxy();handler=h.nwoas_nvme_link_handler
runpy.run_path(str(HERE/'module.py'),init_globals={'hv':h,'p':p})
assert p.calls==[(17,),(16,8448,8)] and len(h.tracers)==2 and h.nwoas_nvme_link_handler is handler
