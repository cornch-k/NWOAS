#!/bin/bash
# S129: S128 plus internal SSD before USB for installed-Windows first boot. PASS: UEFI I/O before EBS.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s129-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[S129] Transport bring-up: mini Windows zone write trial, second namespace in host RAM" | tee -a "$LOG"
