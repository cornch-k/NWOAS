...
spec=importlib.util.spec_from_file_location('s228_ref',S.parent/'nvme-s139/controller.py');ref=importlib.util.module_from_spec(spec);spec.loader.exec_module(ref)
lib=C.CDLL(str(S/'out/frontend.dylib'));lib.configure.argtypes=[C.c_uint64,C.c_uint,C.c_uint64,C.c_uint];lib.guest_write.argtypes=[C.c_uint64,C.c_void_p,C.c_uint];lib.memory_view.restype=C.c_void_p;lib.inspect.argtypes=[C.c_uint];lib.inspect.restype=C.c_uint64
lib.access_read.argtypes=[C.c_int,C.c_uint,C.c_uint];lib.access_read.restype=C.c_uint64;lib.access_write.argtypes=[C.c_int,C.c_uint,C.c_uint,C.c_uint64]
...
def compare():
 ...
 for off,w in [(0,64),(8,32),(0xc,32),(0x10,32),(0x14,32),(0x1c,32),(0x24,32),(0x28,64),(0x30,64)]:
  assert lib.access_read(0,off,w)==c.read(off,w),(checks,'control',hex(off),lib.access_read(0,off,w),c.read(off,w))
 for off,w in [(0,32),(4,32),(0x10,64),(0x2c,32),(0x3c,32)]:assert lib.access_read(1,off,w)==c.pci_read(off,w)
 checks+=1
 ...
 for seed in [227,228,229]:
  ...
  for j in range(700):
   ...
   if rng.randrange(9)==0:
    off=rng.choice([0xc,0x10]);v=rng.choice([0,1,0xffffffff]);c.write(off,v,32);lib.access_write(0,off,32,v);compare();continue
   if rng.randrange(11)==0:
    v=rng.choice([0,7,0x400,0x407]);c.pci_write(4,v,16);lib.access_write(1,4,16,v);compare();continue
