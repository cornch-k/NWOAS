 ...
 *
 * S185 review test: compiles the REAL NwoasHardwareBootRtcLib.c (unmodified,
 * from ../NwoasHardwareBootRtcLib/) against host shim headers and drives
 * LibRtcInitialize / LibGetTime with a mock SPMI controller, mock ADT and a
 * mock physical counter. Nothing touches hardware or any file outside
 * rtc-s185/review-tests/.
 */
#include <stdarg.h>
#include <stdio.h>
#include <stdlib.h>
...
VOID ShimDebugPrint (UINTN Level, CONST CHAR8 *Fmt, ...)
{
  ...
  va_end (ap);
  g_debug_calls++;
  if (getenv ("S185_REVIEW_VERBOSE")) fputs (g_last_debug, stdout);
}
...
static void expect_not_ready (const char *why, unsigned expected_cmd_writes)
{
  EFI_TIME t; EFI_TIME_CAPABILITIES cap;
  EFI_STATUS st = init ();
  CHECK_EQ (st, EFI_SUCCESS); /* init never fails the driver; NOT_READY is reported by GetTime */
  CHECK_EQ (g_bus.cmd_writes, expected_cmd_writes);
  CHECK_EQ (g_bus.bad_addr, 0);
  efi_time_fill_garbage (&t); memset (&cap, 0xff, sizeof cap);
  st = LibGetTime (&t, &cap);
  if (st != EFI_NOT_READY) fprintf (stderr, "  case: %s\n", why);
  CHECK_EQ (st, EFI_NOT_READY);
  CHECK_EQ (cap.Resolution, 1); CHECK_EQ (cap.Accuracy, 100000000); CHECK_EQ (cap.SetsToZero, 0);
  CHECK (strstr (g_last_debug, "GetTime NOT_READY") != NULL);
}

static void test_topology_guard (void)
{
  puts ("topology guard: any ADT mismatch => no CMD write, GetTime NOT_READY");
  reset_all (); g_chip_id = 0x8112;             expect_not_ready ("chip-id T8112", 0);
  reset_all (); g_chip_id_len = 2;              expect_not_ready ("chip-id short", 0);
  reset_all (); g_hide_chosen = 1;              expect_not_ready ("no /chosen", 0);
  reset_all (); g_hide_ctrl = 1;                expect_not_ready ("no nub-spmi", 0);
  reset_all (); g_hide_pmu = 1;                 expect_not_ready ("no spmi-pmu", 0);
  reset_all (); g_pmu_reg = 14;                 expect_not_ready ("sid 14", 0);
  reset_all (); g_hide_info_rtc = 1;            expect_not_ready ("no info-rtc", 0);
  reset_all (); g_info_rtc = 0xd000;            expect_not_ready ("info-rtc alarm ctrl", 0);
  reset_all (); g_hide_scr = 1;                 expect_not_ready ("no info-rtc_scrpad", 0);
  reset_all (); g_info_scr = 0xd008;            expect_not_ready ("scrpad wrong", 0);
  reset_all (); g_ctrl_reg[0] = 0x23d0d9300ull; expect_not_ready ("absolute instead of arm-io relative reg", 0);
  reset_all (); g_ctrl_reg[0] = 0x3d0d9400ull;  expect_not_ready ("wrong controller addr", 0);
  reset_all (); g_ctrl_reg[1] = 0x4000ull;      expect_not_ready ("wrong controller size", 0);
  reset_all (); g_ctrl_reg_len = 8;             expect_not_ready ("reg too short", 0);
...
{
  puts ("bus/decode failures: explicit NOT_READY, bounded writes, no drain");
  reset_all (); g_bus.tx_count = 1;               expect_not_ready ("FIFO TX busy", 0);
  reset_all (); bus_push (0x12345678u);           expect_not_ready ("FIFO RX queued", 0); CHECK (!bus_empty ());
  reset_all (); g_bus.never_reply = 1;            expect_not_ready ("no reply", 1);
  ...
  reset_all (); g_bus.header = 0x003f0f3cu;       expect_not_ready ("header echo", 1);
  reset_all (); g_bus.extra_word = 1;             expect_not_ready ("extra reply word", 1); CHECK (!bus_empty ());
  reset_all (); g_bus.counter_seq[1] = g_bus.counter_seq[0] - 1; expect_not_ready ("counter backwards", 3);
  reset_all (); g_bus.counter_seq[1] = g_bus.counter_seq[0] + NWOAS_SERA_MAX_COUNTER_SPAN + 1; expect_not_ready ("counter span", 3);
  reset_all (); g_bus.offset = 0; g_bus.counter_seq[0] = g_bus.counter_seq[1] = (946684799ull << 16); expect_not_ready ("1999-12-31", 3);
  reset_all (); g_bus.offset = 0; g_bus.counter_seq[0] = g_bus.counter_seq[1] = (4102444800ull << 16); expect_not_ready ("2100-01-01", 3);
  reset_all (); g_bus.offset = (1ull << 47); g_bus.counter_seq[0] = g_bus.counter_seq[1] = 0; expect_not_ready ("negative 49-bit sum", 3);
  ...
  reset_all (); g_clk.frq = 999999; expect_not_ready ("cntfrq too low", 0); CHECK_EQ (g_bus.status_reads, 0);
  reset_all (); g_clk.frq = 0;      expect_not_ready ("cntfrq zero", 0);
}
...
static void test_integration_window_guard (void)
{
  ...
  reset_all (); g_clk.jump_at_read = 18; g_clk.jump = 24000000ull * 2; expect_not_ready ("span > 1 s (after)", 3);
...
static void test_reinit_resets_state (void)
{
  ...
  CHECK_EQ (init (), EFI_SUCCESS);
  CHECK_EQ (LibGetTime (&t, NULL), EFI_NOT_READY);
}
