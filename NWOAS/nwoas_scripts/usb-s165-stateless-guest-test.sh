...
HV="$ROOT/nwoas_scripts/usb-s165/m1n1-s165-stateless-gap50.bin"
PAYLOAD="$M1N1/m1n1-payload-s135-8cpu-speed.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s165-stateless-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 2742051583cb237ed8f2a1060c3fbeb9b69882a7dc8f9b26316a75fdb3a9c189 ]
...
echo '[S165 stateless] CONTROL: S163 gap50us + S135 USB-C visibility and D81/D83 initialization' | tee -a "$LOG"
