#!/bin/bash
# S6-D73: fix HPM ASCII index decoding in init/restore on D66 base.
...
HV="$M1N1/build/m1n1-d73-usbc-hse-rings.bin"
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-d73-usbc-hse-rings-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "d0f8483dc1434183d69eec4a3e37f83cdc94263f4fe525e7e18562a604603c2b" ]
...
echo "[DESIGN] D73 one HSE snapshot of USB-C event ring and DART fault state on D72" | tee -a "$LOG"
