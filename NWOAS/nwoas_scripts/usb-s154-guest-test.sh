#!/bin/bash
# S154: S153 plus USB-C pending-poll event-ring visibility.
set -eu
...
HV="$M1N1/build/m1n1-s154-usbc-poll-coherence.bin"
PAYLOAD="$M1N1/m1n1-payload-s135-8cpu-speed.bin"
USBC="$M1N1/pcie_emul-d81-usbc-payload.py"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s154-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 16d222cfd4996e53d141dba3fcd12d7a907aca9b8b5e58c2f420f5867b21868f ]
...
echo '[S154] S153 + USB-C event invalidate on pending status polls' | tee -a "$LOG"
