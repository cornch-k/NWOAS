...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/cpufreq-s189-native-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo '[S189] Native UEFI ReadyToBoot P12; host observer read-only; S187 RAM/RTC/USB-C control' | tee -a "$LOG"
