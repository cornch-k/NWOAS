#!/bin/bash
# S6-D56: D51 runtime plus a one-shot EL2 framebuffer baseline comparison.
# ONLY VARIABLE from D55: observational 320x180 non-background mask added to the +120s probe.
# PASS: FBMASK is complete and can be decoded into a monochrome preview; no runtime writes.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-iponly-fbmask-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D56 D55 runtime unchanged; add 320x180 non-background framebuffer mask" | tee -a "$LOG"
