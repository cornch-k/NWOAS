#!/bin/bash
# S6-D11: ONLY variable is eight trailing zero bytes in the surface record.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-surface172-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
try:
    ...
    link=TahoeLink(i,p,(1280,1280,3072,3072),os.environ['NWOAS_PROTOCOL_LOG'])
    link.sequence,link.command_sequence=8,7
    u=ProxyUtils(p)
