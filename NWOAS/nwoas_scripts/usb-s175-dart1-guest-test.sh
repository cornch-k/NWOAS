#!/bin/bash
# S175: same S174 payload, DART1/SID1 translate preserving DAPF; no D83/LOWALIAS.
...
HV="$ROOT/nwoas_scripts/usb-s175/m1n1-s175-dart1-gap50.bin"
PAYLOAD="$ROOT/nwoas_scripts/usb-s174/m1n1-payload-s174-8cpu-usbc.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s175-dart1-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 6758060953a01faa65f435c3dcaaaced7c5868d554048a2c3fde794205d5cfbe ]
...
echo '[S175] S174 RAM/USB-C; DART1 SID1 translate, DAPF preserved, no pointer rewrites' | tee -a "$LOG"
