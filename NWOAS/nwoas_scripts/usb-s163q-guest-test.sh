...
HV="$ROOT/m1n1_windows-s159/build/m1n1-s163-eoi-gap200.bin"
PAYLOAD="$M1N1/m1n1-payload-s139-8cpu-usba.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s163q-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 5c14b870824bffd1a62d81e7a18edb579801323b484194e5b22905711156a173 ]
...
echo '[S163] IRQ900 EOI reassert gap200us, S162 CQ probe, 8 cores USB-A' | tee -a "$LOG"
