#!/bin/bash
# S6-D21: one standard service-open call (public group4/cmd6) before AV queries.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-av-open-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
try:
    ...
    link=TahoeLink(i,p,(384,384,1024,1024),prefix,**{k:state[k] for k in ('boot_base','shared','syslog_base','asc_base','endpoint')},interface=5,half_size=0x2000)
    link.sequence,link.command_sequence=2,1
    msg=struct.pack('<2xHIII48x',4,6,32,0x69706378)+bytes(32)
    response=link.command(msg,96)
    print('[FACT] Standard service OPEN status0')
