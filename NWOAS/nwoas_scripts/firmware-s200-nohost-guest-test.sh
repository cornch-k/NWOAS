#!/bin/bash
# S200: S192 native RTC/P12 and bounded HPD wait, with S193 host CPU init omitted.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/firmware-s200-nohost-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo '[S200] Native RTC/P12 plus bounded HPD; no S140 host CPU init; S163 gap50' | tee -a "$LOG"
