...
static bool nwoas_fl_cache_guest_range(u64 ipa, size_t len, bool clean)
{
    while (len) {
        size_t chunk = PAGE_SIZE - (ipa & (PAGE_SIZE - 1));
        if (chunk > len)
            chunk = len;
        u64 pa = nwoas_ipa_to_pa(ipa);
        if (!pa || !nwoas_in_dram(pa, chunk))
            return false;
        nwoas_cache_maint(pa, chunk, clean);
        ipa += chunk;
        len -= chunk;
    }
    return true;
...
// When usbxhci sees EINT/IP, invalidate the event-ring segment containing ERDP so the
// CPU reads the FL1100's DMA'd completions. Windows normally programs four segments;
// S151 always maintained ERST[0], while S155 maintained all four on every pending-bit
// read. S156 is a bounded A/B candidate reducing maintained bytes, not proof that
// cache cost caused S155's stall. Hardware ERDP can lag the software consumer; a
// driver draining across segments without an ERDP update can still see stale TRBs.
// At the final cache line, also invalidate the next ERST segment for wraparound.
static void nwoas_fl_inval_evt(struct exc_info *ctx)
{
    ...
    const u64 bar = 0x6c0000000UL;
    u64 rtsoff = 0, erstsz_raw = 0, erdp_raw = 0;
    if (!hv_pa_read(ctx, bar + 0x18, &rtsoff, 2))
        return;
    rtsoff &= ~0x1fUL;
    if (!rtsoff || rtsoff >= 0x100000 ||
        !hv_pa_read(ctx, bar + rtsoff + 0x28, &erstsz_raw, 2) ||
        !hv_pa_read(ctx, bar + rtsoff + 0x38, &erdp_raw, 3))
        return;
    u32 segments = erstsz_raw & 0xffff;
    if (!segments || segments > 16)
        return;

    u64 erst = nwoas_fl_erstba & ~0x3fUL;
    size_t table_len = (size_t)segments * 16;
    /* ERST is guest-authored and device-read-only.  Preserve a possibly dirty
     * usbxhci update before reading all segment descriptors; invalidate-only
     * here can discard that update and send us to an older ring. */
    if (!nwoas_fl_cache_guest_range(erst, table_len, true))
        return;

    u64 erdp = erdp_raw & ~0xfUL;
    u64 rings[16] = {0};
    u32 counts[16] = {0};
    u64 selected_ring = 0;
    u32 selected_count = 0;
    u32 selected_segment = 0xffffffff;
    for (u32 i = 0; i < segments; i++) {
        u64 entry_pa = nwoas_ipa_to_pa(erst + (u64)i * 16);
        u64 ring = 0, count_raw = 0;
        if (!entry_pa || !nwoas_in_dram(entry_pa, 16) ||
            !hv_pa_read(ctx, entry_pa, &ring, 3) ||
            !hv_pa_read(ctx, entry_pa + 8, &count_raw, 2))
            return;
        u32 count = count_raw & 0xffff;
        if (!(ring & ~0x3fUL) || (ring & 0x3f) || !count || count > 4096)
            return;
        u64 ring_end = ring + (u64)count * 16;
        if (ring_end < ring)
            return;
        rings[i] = ring;
        counts[i] = count;
        if (erdp >= ring && erdp < ring_end) {
            selected_ring = ring;
    ...
    if (selected_segment == 0xffffffff)
        return;
    if (!nwoas_fl_cache_guest_range(selected_ring,
                                    (size_t)selected_count * 16, false))
        return;

    bool boundary_next = false;
    u64 selected_end = selected_ring + (u64)selected_count * 16;
    if (segments > 1 && selected_end - erdp <= 64) {
        u32 next = (selected_segment + 1) % segments;
        if (!nwoas_fl_cache_guest_range(rings[next],
                                        (size_t)counts[next] * 16, false))
            return;
        boundary_next = true;
    }
    ...
    if (!reported) {
        reported = true;
        printf("HVLOG: S156 FL1100 ERDP segment coherence segments=%u selected=%u "
               "erdp=0x%lx ring=0x%lx count=%u boundary_next=%u\n",
               segments, selected_segment, erdp, selected_ring, selected_count,
               boundary_next);
    }
}
...
static void nwoas_usbc_payload_alias(u64 op, u32 slot, u32 ep)
{
    ...
    if (!dq) return;
    /* D83: xHC output-context DQ remains at original ring while Windows
     * retires/zeros old segments. Follow the last observed live Link target
     * on the next doorbell instead of restarting at the freed first segment. */
    static u64 saved_ctx[32], saved_dq[32], live_segment[32];
    if (saved_ctx[ep]!=ctx || saved_dq[ep]!=dq) {
        saved_ctx[ep]=ctx;saved_dq[ep]=dq;
        live_segment[ep]=nwoas_ipa_to_pa(dq);
    }
    u64 rp=live_segment[ep];
        ...
        switch (FIELD_GET(SPTE_TYPE, pte)) {
            ...
            case SPTE_MAP:
                ...
                if (paddr >= 0x6c0000000UL && paddr < 0x6c0100000UL) {
                    ...
                    if (((off == 0x84 && (val[0] & 0x8)) ||
                         (off == 0x2020 && (val[0] & 0x1))) &&
                        !nwoas_synth_pending && (elr >> 48) == 0xffff)
