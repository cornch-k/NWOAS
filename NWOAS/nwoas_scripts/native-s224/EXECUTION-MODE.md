...
Storage is exposed as a synthetic NVMe controller. Its data path is partly
local C code on the target; control/admin operations and service transport still
use the host. S224 projects supported PCI/NVMe reads into target-local state,
without moving write/admin ownership. S225/S226 are offline-only foundations;
they do not yet produce completions, own DMA/IRQs or remove the host dependency.
...
not a single-core score, and not a measurement of the newer S224 configuration.
...
- hardware-result.json and related bounded test results: S224 hardware evidence.
