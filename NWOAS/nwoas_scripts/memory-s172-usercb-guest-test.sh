...
PAYLOAD="$ROOT/nwoas_scripts/memory-s172/m1n1-payload-s172-8cpu-usba.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/memory-s172-usercb-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = e55e257efb7efec44c3968d4bd73c0aff009e75cc9228d34b9f30a68303cc8c7 ]
...
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/bench-session-s176/link_module.py" | awk '{print $1}')" = f6127acbcbc282b526ba78c7427a4eb5ad446d38f6881c7576c842903c15c21e ]
...
echo '[S172] 8 GiB high RAM plus low window; late P12, S163 gap50us; S168 delivery registered' | tee -a "$LOG"
...
"$PY" -u "$M1N1/proxyclient/tools/run_guest.py" -r --strict-init \
    ...
    -m "$ROOT/nwoas_scripts/bench-session-s176/link_module.py" \
