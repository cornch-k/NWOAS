#!/bin/bash
# S151: S150 NVMe link plus deterministic visibility for real FL1100 events.
set -eu
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s151-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo '[S151] S150 NVMe link; real FL1100 EINT invalidates device-written event ring' | tee -a "$LOG"
