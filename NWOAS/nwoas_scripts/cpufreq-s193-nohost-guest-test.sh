#!/bin/bash
# S193: exact S189 control with redundant host CPU initialization omitted.
# Guest m1n1 initializes clusters; native UEFI requests P12; host only observes.
...
PAYLOAD="$ROOT/nwoas_scripts/cpufreq-s189/m1n1-payload-s189-native-p12.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/cpufreq-s193-nohost-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = 3b66d5d858bb93f07772f7ac0cf185329ea2daf05438b888d076ed8f3e6836b2 ]
...
echo '[S193] S189 exact payload/HV; S140 host CPU init omitted; captures read-only' | tee -a "$LOG"
