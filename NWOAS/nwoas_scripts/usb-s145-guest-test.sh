#!/bin/bash
# S145: S144 split CDC with guest UART reduced to filtered HVLOG lines.
set -eu
...
HV="$M1N1/build/m1n1-s145-filtered-cdc.bin"
PAYLOAD="$M1N1/m1n1-payload-s139-8cpu-usba.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s145-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo '[S145] S141 CPU/NVMe + split CDC: NY1 proxy-only, NY3 filtered diagnostics' | tee -a "$LOG"
