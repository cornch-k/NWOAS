#!/bin/bash
# S6-D41: force FL1100 HCCPARAMS1.AC64=0 in the guest-visible BAR and leave
# Windows's xHCI ring ownership intact. The D39/D40 low-ring seed/mirror is off.
# PASS: FL32 marker plus kernel DCBAAP/ERSTBA/CRCR below 4GB, then real event
# consumption/ERDP progress and no 10.1s HCRST recurrence. Cursor is final proof.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D41 AC64=0 low-DMA allocation; event seed/mirror disabled" | tee -a "$LOG"
