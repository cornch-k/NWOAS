#!/bin/bash
# S6-D20: one read-only AV getPower call through the measured interface5.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-av-power-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
try:
    ...
    link=TahoeLink(i,p,tuple(state['pointers']),prefix,**{k:state[k] for k in ('boot_base','shared','syslog_base','asc_base','endpoint')},interface=5,half_size=0x2000)
    body=struct.pack('<QBBB5x',0,0x12,0,1)
    wptr=link.tx.write(bytes(8)+struct.pack('<BBHI',0,0,5,len(body))+body)
    link.mailbox_send((0xa2<<48)|wptr,0x24)
