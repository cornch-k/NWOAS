#!/bin/bash
# S6-D47: D42's AC64=0 low-DMA allocation plus a UEFI payload whose live
...
# ONLY VARIABLE from D46: dump architectural SError and complete vGIC state at the common bugcheck probe.
# PASS: BUGPROBE armed, followed by BUGCHECK code/parameters if Windows enters
# the WHEA blue-screen path. USB success remains real controller events plus
# advancing ERDP; cursor movement remains the final HID proof.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-serror-state-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D47 D46 baseline plus one-shot SError/vGIC state dump at bugcheck" | tee -a "$LOG"
