#!/bin/bash
# S214: S192 native RTC/P12 and bounded HPD wait, with S193 host CPU init omitted.
...
HV="$ROOT/nwoas_scripts/nvme-s208/out/m1n1-s208-eoipc-gap50.bin"
PAYLOAD="$ROOT/nwoas_scripts/firmware-s192/m1n1-payload-s192-native-rtc-p12.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/nvme-s214-eoipc50-control-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = f801c5bec4e2972a0011b594ec8fb9201406dd7a82180631f4328f6d2779fa16 ]
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = b67313c48e057e3c1e608521b69a611e129328c04554785e429f4a4211d4c7c9 ]
...
echo '[S214] Native RTC/P12 plus bounded HPD; no S140 host CPU init; S163 gap50' | tee -a "$LOG"
