...
root=Path('/Volumes/X31/NWOAS');repo=root/'apple_silicon_platforms_mu';out=root/'nwoas_scripts/uefi-s129'
...
opts=base/'Library/MsBootOptionsLib/MsBootOptionsLib.c'
original={p:p.read_bytes() for p in (hide,bm,inf,pci,opts)}
...
assert s.count(usb+'\n'+ssd)==1
s=s.replace(usb+'\n'+ssd,'  DEBUG ((DEBUG_ERROR, "HVLOG: S129 internal SSD boot option first\\n"));\n'+ssd+'\n'+usb,1)
candidate[opts]=s.encode()
...
try:
 ...
 payload=old[:1376256]+fd;target=root/'m1n1_windows/m1n1-payload-s129-ssd-first.bin';target.write_bytes(payload)
 manifest={'payload':str(target),'sha256':hashlib.sha256(payload).hexdigest(),'bytes':len(payload),'change':'S128 plus Internal Storage registered before USB in temporary firmware boot order; original sources restored; installed OS boot untested'}
