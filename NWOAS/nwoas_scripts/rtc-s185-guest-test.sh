...
PAYLOAD="$ROOT/nwoas_scripts/rtc-s185/m1n1-payload-s185-hwrtc.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/rtc-s185-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = fa8711f5446f3f135cda50648fe3121a6dcee6e82103f458033263bb5d38da83 ]
...
echo '[S185] native UEFI boot RTC reader; no host time seed; S180 validated RAM/8core/USB-A, S16350us lateP12' | tee -a "$LOG"
