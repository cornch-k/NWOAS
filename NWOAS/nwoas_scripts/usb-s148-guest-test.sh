#!/bin/bash
# S148: S147 1 MiB NVMe plus target-side hot-path diagnostic suppression.
set -eu
...
HV="$M1N1/build/m1n1-s148-perf-quiet-nvme-1m.bin"
PAYLOAD="$M1N1/m1n1-payload-s139-8cpu-usba.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s148-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 8628b608e8b65299ae6ad6162377e19525f8731c2bea1e833a1b2b77a5e019c0 ]
...
echo '[S148] S147 + compiled-out timer/USB/ISR hot-path diagnostics' | tee -a "$LOG"
