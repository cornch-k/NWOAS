...
OUT=ROOT/'nwoas_scripts/memory-s180'
...
inner=inner.replace('OUT = ROOT / "nwoas_scripts/smp-s131"','OUT = ROOT / "nwoas_scripts/memory-s180/s131-build"')
inner=inner.replace('target = ROOT / "m1n1_windows/m1n1-payload-s131-8cpu.bin"',
                    'target = ROOT / "nwoas_scripts/memory-s180/m1n1-payload-s180-10g-usba.bin"')
...
try:
    ...
    payload=OUT/'m1n1-payload-s180-10g-usba.bin'
    manifest={'payload':str(payload),'bytes':payload.stat().st_size,
              'sha256':hashlib.sha256(payload.read_bytes()).hexdigest(),
              'change':'S161 normalization plus first 10 GiB high RAM kept; 8 CPUs; XHC1 hidden',
