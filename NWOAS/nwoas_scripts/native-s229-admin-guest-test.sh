#!/bin/bash
# S229: target-local control/admin, S223 payload; existing data path and NS2 transport retained.
...
HV="$ROOT/nwoas_scripts/native-s229/out/m1n1-s229-admin.bin"
PAYLOAD="$ROOT/nwoas_scripts/loader-s223/m1n1-payload-s223-fd-tail.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/native-s229-admin-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 6ae35d6a13c0843afc7b0d5fd051c1c215cf7b84e101f9d99acc1f6911d7b70b ]
...
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/native-s229/query_module.py" | awk '{print $1}')" = 2254d609866c4229659fcb4bc13cdf4b42e2e5fdc79b035a17ba5cdce8805b29 ]
...
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/native-s229/module.py" | awk '{print $1}')" = 444d31169917ad5ed0c62e734f88fc7623b9832015e11d58cba45e4afa6cc2d0 ]
...
echo '[S229] Local control/admin; S223 payload; host NS2 remains' | tee -a "$LOG"
...
"$PY" -u "$M1N1/proxyclient/tools/run_guest.py" -r --strict-init \
    ...
    -m "$ROOT/nwoas_scripts/native-s229/module.py" \
    -m "$ROOT/nwoas_scripts/native-s229/query_module.py" \
