/* S115: read-only Windows ARM64 file hash diagnostic, using Windows CNG SHA256.
...
void mainCRTStartup(void){
 H alg=0,hash=0,file=(H)-1,heap=GetProcessHeap();U8 *object=0,*buffer=0,digest[32];U32 len=0,got=0,rc=2;long long size=0;U64 total=0,started=GetTickCount64(),next=268435456;static W path[2048],skip[2048],expect[65],mode[16];int direct=0;W *cmd=GetCommandLineW();
 output=GetStdHandle((U32)-11);emit("NWOAS S115 ARM64 SHA256, input read-only\r\n");
 ...
 file=CreateFileW(path,0x80000000,7,0,3,direct?0x28000000:0x08000000,0);
 ...
 while(total<(U64)size){
  ...
  if(!got||got>(U64)size-total){emit("FAIL short or oversized read before EOF\r\n");goto end;}
