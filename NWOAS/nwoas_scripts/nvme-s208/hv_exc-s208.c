...
// PSCI-SMC verbose, UEFI/bmgfw UART re-dumps, VGATE timer-gate trace). The direct tether is USB
// CDC bulk rather than a baud-limited UART, but formatting and transmitting diagnostics still
// runs synchronously on EL2 hot paths. NWOAS_HOTPATH_LOG separately controls timer/USB/ISR probes.
// Set this to 0 only for a dedicated diagnostic build.
...
{
    u64 dart0 = 0x502f00000UL;
    u64 dart1 = 0x502f80000UL;
    printf("[usb-s153] %s gsts=%x buserr=%x:%08x "
           "dart0=%x@%x:%08x dart1=%x@%x:%08x\n",
           phase, read32(NWOAS_XHCI_BASE + 0xc118),
           read32(NWOAS_XHCI_BASE + 0xc134), read32(NWOAS_XHCI_BASE + 0xc130),
           read32(dart0 + 0x40), read32(dart0 + 0x54), read32(dart0 + 0x50),
           read32(dart1 + 0x40), read32(dart1 + 0x54), read32(dart1 + 0x50));
}
...
{
    nwoas_nvme_irq_host_level = !!level;
    sysop("dsb sy");
}
void nwoas_nvme_set_fast_irq(u64 level)
{
    nwoas_nvme_irq_fast_level = !!level;
    sysop("dsb sy");
}
static void hv_bridge_nvme_irq(void)
{
    int cpu = hv_pinned_cpu == -1 ? boot_cpu_idx : hv_pinned_cpu;
    if ((int)smp_id() != cpu || !(nwoas_nvme_irq_host_level || nwoas_nvme_irq_fast_level))
        return;
...
static void hv_bridge_usb_xhci_irq(void)
{
    ...
    /* S153: the USB-C xHC writes its event ring through non-coherent DMA.
     * Publish device completions to the guest before either the descriptor
     * observer or Windows' ISR can consume them.  Invalidate only; the event
     * segments are controller-owned. */
    bool global_ie = !!(read32(op_base + 0x00) & BIT(2));
    if (armed && pending && global_ie && hv_vgic3_spi_enabled(NWOAS_USB_SPI)) {
        extern void nwoas_usbc_event_coherence(u64 rt, bool before_run);
        nwoas_usbc_event_coherence(rt_base, false);
    ...
    if (hv_vgic3_spi_enabled(NWOAS_USB_SPI) && !usbc_ready_time) {
        usbc_ready_time = now;
        nwoas_usbc_error_snapshot("spi-enable");
    }
    ...
    if (!usbc_hse_dumped && (usbsts & 4u) && hv_vgic3_spi_enabled(NWOAS_USB_SPI)) {
        ...
        nwoas_usbc_error_snapshot("first-hse");
        extern void nwoas_probe_usbc_rings(u64, u64, u64);
        ...
        */
        ...
        case SYSREG_ISS(SYS_PMCCNTR_EL0): {
            if (is_read) {
                ...
                // only the first 32 reads. That is enough to identify the phase/caller
                // without making eight cores contend on a 115200-baud diagnostic UART.
