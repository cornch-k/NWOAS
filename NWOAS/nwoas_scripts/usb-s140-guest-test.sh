#!/bin/bash
# S140: initialize T8103 CPU P-states, then boot the stable S139 experiment.
set -eu
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s140-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
shasum -a 256 "$HV" "$PAYLOAD" "$ROOT/nwoas_scripts/cpufreq-s140/init.py" "$ROOT/nwoas_scripts/nvme-s139/controller.py" >> "$LOG"
echo '[S140] S139 baseline + verified T8103 cpufreq initialization' | tee -a "$LOG"
