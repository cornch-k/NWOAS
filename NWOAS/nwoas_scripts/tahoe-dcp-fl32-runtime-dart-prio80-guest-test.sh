#!/bin/bash
# S6-D50: D49 baseline with FL1100 IRQ698 injected at the guest-programmed
# distributor priority 0x80 instead of the forced 0x00 priority.
# ONLY BEHAVIORAL VARIABLE from D49: IRQ698 LR priority.
# PASS: real controller events are consumed without WHEA 0x124/0x12; command
# ring activity or further Windows USB progress is decisive positive evidence.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-prio80-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D50 D49 baseline; only behavioral variable is IRQ698 priority 0x00 -> guest-programmed 0x80" | tee -a "$LOG"
