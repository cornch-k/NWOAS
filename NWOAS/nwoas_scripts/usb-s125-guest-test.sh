#!/bin/bash
# S124 transport-only: Windows zone writes enabled, no automatic install. S103/S102 unchanged.
...
PAYLOAD="$M1N1/m1n1-payload-s125-uefinvme.bin"
HOOK="$M1N1/tahoe_dcp_guest_hook.py"
MODULE="$M1N1/pcie_emul-d81-usbc-payload.py"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s125-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = "bf00b5a4bc9ed53abf7e512854ca5408e0352ead16f9511626937aaf1e9ab43a" ]
if /usr/sbin/lsof -t "$DEV" >/dev/null 2>&1; then
    echo "STOP: serial busy"
    exit 1
fi
shasum -a 256 "$HV" "$PAYLOAD" "$HOOK" "$MODULE" >> "$LOG"
echo "[S125] Transport bring-up: mini Windows zone write trial, second namespace in host RAM" | tee -a "$LOG"
