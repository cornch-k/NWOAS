#!/bin/bash
# S6-D46: D42's AC64=0 low-DMA allocation plus a UEFI payload whose live
...
# ONLY VARIABLE from D45: accept a stable asserted HDMI HPD after the full settle window for unattended testing.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-headless-commonbug-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D46 D45 common bugcheck probe plus stable-HPD headless DCP handoff" | tee -a "$LOG"
