#!/bin/bash
# S202: S192 native RTC/P12 and bounded HPD wait, with S193 host CPU init omitted.
# S194 25us NVMe candidate; host reads cluster state without initializing it.
...
HV="$ROOT/nwoas_scripts/nvme-s194/m1n1-s194-eoi-gap25.bin"
PAYLOAD="$ROOT/nwoas_scripts/firmware-s192/m1n1-payload-s192-native-rtc-p12.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/nvme-s202-gap25-nohost-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 3801b7164320cb2087ca35293ad9058174d797450af81a80d71fec5ae9135b06 ]
...
echo '[S202] S200 exact payload/host init; only NVMe EOI gap changes to25us' | tee -a "$LOG"
