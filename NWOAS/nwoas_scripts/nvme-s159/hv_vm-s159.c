...
static bool nwoas_nvme_fastpath_mmio(struct exc_info *ctx, u64 ipa, u64 *val, bool write,
                                     u64 width)
{
    (void)ctx; /* S159 executes queued I/O from the tick's context. */
    ...
    if (off == 0x1008) {
        if (*val < nwoas_nvme_fp.sq_size) {
            ...
            /* S159 experiment: publish the producer cursor immediately.
             * The existing HV tick consumes in SQ order; CQ phase publication
             * still follows physical completion. No completion is fabricated. */
        } else {
            nwoas_nvme_fp.errors++;
        }
        return true;
