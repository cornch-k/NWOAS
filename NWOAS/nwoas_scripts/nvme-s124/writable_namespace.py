...
class WindowWritableNamespace(ReadOnlyNamespace):
    ...
    def set_cache(self,enabled):
        if self.cache_enabled and not enabled:self.flush()
        self.cache_enabled=bool(enabled)

    def get_cache(self):return int(self.cache_enabled)

    def completed_write(self,cdw):
        if (cdw[2] & (1<<30)) or not self.cache_enabled:
            try:self.flush()
    ...
    def admin(self,command):
        ...
        if r.status==SUCCESS and command[0]==6 and command[40]==1:
            d=bytearray(r.data);d[525]=1 # volatile write cache present
    ...
    def _io(self,command,mem=None):
        ...
        slba=cdw[0]|(cdw[1]<<32)
        nlb=(cdw[2]&0xffff)+1
        # S124: FUA forces durability; normal writes may use advertised volatile cache.
        ...
        if gpt_write:return Result(WRITE_TO_RO) # no partition-table mutations in persistence trial
        if mem is None:return Result(INVALID_FIELD)
        if not gpt_write and self._try_direct(True,slba,nlb,command):return self.completed_write(cdw)
        ...
        return self.completed_write(cdw)
