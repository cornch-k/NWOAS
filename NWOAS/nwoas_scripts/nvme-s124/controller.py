...
class Controller:
    ...
    def reset(self):
        ...
        self.sq={};self.cq={};self.features={6:1};self.aer=[]
        self.irq(False)
    ...
    def read(self,offset,width):
        ...
        # MQES255, CQR1, timeout10s, NVM command set, 4KiB pages.
        struct.pack_into('<Q',b,0,(self.MAX_DEPTH-1)|(1<<16)|(20<<24)|(1<<37))
    ...
    def write(self,offset,value,width):
        ...
        if offset==0xc:self.mask|=value;self.update_irq()
            ...
            if (value>>14)&3 and not(self.csts&2) and (self.csts&12)!=8:
                self.csts=(self.csts&~12)|4
                try:self.ns.flush()
                except (OSError,TimeoutError) as e:return self.fatal('S124 shutdown flush failed: '+str(e))
                self.csts=(self.csts&~12)|8
                self.log('S124 SHUTDOWN flush complete')
    ...
    def admin(self,cmd):
        ...
        if op in (9,10):
            ...
            if fid==6 and hasattr(self.ns,'set_cache'):
                if dw[0]&~0x7ff or any(dw[2:]):return Result(INVALID_FIELD)
                sel=(dw[0]>>8)&7
                if op==10:
                    if sel==1:return Result(SUCCESS,result=1)
                    if sel==3:return Result(SUCCESS,result=4) # changeable, not saveable
                    if sel!=0:return Result(INVALID_FIELD)
                if op==9 and (sel or dw[1]&~1):return Result(INVALID_FIELD)
                if op==9:
                    try:self.ns.set_cache(bool(dw[1]))
                    except (OSError,TimeoutError):return Result(6)
                return Result(SUCCESS,result=self.ns.get_cache())
