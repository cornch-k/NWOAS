#!/bin/bash
# S6-D44: D42's AC64=0 low-DMA allocation plus a UEFI payload whose live
...
# ONLY VARIABLE from D43: one-shot KeBugCheckEx probe in the outer m1n1.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-bugprobe-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D44 D43 runtime-DART baseline plus one-shot KeBugCheckEx probe" | tee -a "$LOG"
