#!/bin/bash
# NWOAS §3.2 baseline: deferred-v1 + pcie_emul-exp5-fldiag3.py (adds AC64 + dwc3 GCTL reads).
...
PAYLOAD=m1n1-payload-hidehighram-v1.bin
MODULE=pcie_emul-exp5-fldiag3.py
VDM="$ROOT/macvdmtool/macvdmtool"
LOG="$ROOT/nwoas_scripts/logs/hidehighram-fldiag3.log"
: > "$LOG"
echo "=== deferred hidehighram(CandA)+fldiag3 start $(date '+%H:%M:%S') payload=$PAYLOAD module=$MODULE ===" | tee -a "$LOG"
...
for i in $(seq 1 540); do   # ~45min budget
  ...
  if grep -qa "fldiag3. breaker done\|fldiag3. === dump #4 done\|fldiag3. ★CMD.MEM=0" "$LOG"; then
    echo "=== ★FL1100 DUMPS DONE @iter$i $(date '+%H:%M:%S') ===" | tee -a "$LOG"; sleep 6; break
  fi
  [ $((i % 12)) = 0 ] && echo "  [iter$i $(date '+%H:%M:%S')] reads=$(grep -ca UsbBootReadBlocks $LOG) kernel=$(grep -ca 'elr=0xfffff8' $LOG) dumps=$(grep -ca 'fldiag3. === DUMP' $LOG)" | tee -a "$LOG"
done
