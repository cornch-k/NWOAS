#!/bin/bash
# S6-D51: D50 baseline with IRQ698 bridge pending derived only from IMAN.IP.
# ONLY BEHAVIORAL VARIABLE from D50: ignore stale USBSTS.EINT after IP clears.
# PASS: the post-consume empty IRQ storm stops and Windows writes an xHCI command
# TRB or otherwise advances beyond the USB initialization stall without WHEA.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-iponly-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D51 D50 baseline; only behavioral variable is bridge pending IMAN.IP || USBSTS.EINT -> IMAN.IP" | tee -a "$LOG"
