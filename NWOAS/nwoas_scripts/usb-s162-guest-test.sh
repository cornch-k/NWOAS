...
HV="$ROOT/m1n1_windows-s159/build/m1n1-s162-cq-map.bin"
PAYLOAD="$M1N1/m1n1-payload-s139-8cpu-usba.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s162-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 6fea29b58a8803077e0d66bef32a61dad5e8213e6317401d85f7c3febedc7a37 ]
...
echo '[S162] Read-only CQ mapping probe on S160b, 8 cores USB-A' | tee -a "$LOG"
