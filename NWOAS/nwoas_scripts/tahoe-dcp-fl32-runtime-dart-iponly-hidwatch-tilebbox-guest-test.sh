#!/bin/bash
# S6-D61: D60 watcher with tile-localized post-input framebuffer sampling.
# ONLY VARIABLE from D60: capture 16x16 tile hashes of the stable Setup
# surface and report changed-tile count/bbox after a physical HID completion.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-iponly-hidwatch-tilebbox-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D61 D60 runtime unchanged; add 16x16 tile count/bbox to post-HID framebuffer samples" | tee -a "$LOG"
