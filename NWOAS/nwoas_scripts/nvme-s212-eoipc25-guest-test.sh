#!/bin/bash
# S212: S203 source-built compat prefix and original 256.875MiB header; diagnostic-only HV change.
...
HV="$ROOT/nwoas_scripts/nvme-s208/out/m1n1-s208-eoipc-gap25.bin"
PAYLOAD="$ROOT/nwoas_scripts/loader-s197/out/compat/m1n1-payload-s197-compat.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/nvme-s212-eoipc25-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 1f1291dd354ab202faaceccd05635b0d1cac7666f32bf808c49807db0bec5bde ]
...
echo '[S212] S197 compat prefix; S208 architectural EOI PC diagnostic, gap25' | tee -a "$LOG"
