#!/bin/bash
# S6-D40: ONLY USB variable versus D39 is one-shot SPI 698 delivery for a
# mirrored low-ring event. Delivery targets the first guest CPU with IRQs
# unmasked and remains deduplicated until Windows advances ERDP.
# PASS: EVTMIRROR IRQ698, then a guest ERDP write and USB progress beyond the
# old 10.1s HCRST wall. A visible Windows installer remains final success.
# Safety: no reboot and one chainload; caller logs one bootstrap reboot first.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-event-mirror-irq-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D40 mirror plus one-shot IRQ698 on first IRQ-enabled CPU" | tee -a "$LOG"
