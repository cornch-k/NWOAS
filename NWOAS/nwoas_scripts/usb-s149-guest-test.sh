#!/bin/bash
# S149: S148 plus target-side synthetic NVMe I/O queue processing.
set -eu
...
HV="$M1N1/build/m1n1-s149-target-nvme-fastpath.bin"
PAYLOAD="$M1N1/m1n1-payload-s139-8cpu-usba.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s149-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 345d9678d2f3f2d1828522d20f87cb35d5892747f3aac66303840859b15236ea ]
...
echo '[S149] target-side NVMe I/O fastpath; Python retains admin queue' | tee -a "$LOG"
