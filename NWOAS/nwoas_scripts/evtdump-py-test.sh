...
MODULE=pcie_emul-exp5-evtdump-py.py
STAMP=$(date '+%H%M')
LOG="$ROOT/nwoas_scripts/logs/evtdump-py-$STAMP.log"
: > "$LOG"
...
for i in $(seq 1 600); do
  ...
  if grep -qa "\[evtpy\] breaker done" "$LOG"; then
    echo "=== ★breaker done @iter$i $(date '+%H:%M:%S') ===" | tee -a "$LOG"; sleep 6; break
  fi
  if [ $((i % 12)) = 0 ]; then
    echo "  [iter$i $(date '+%H:%M:%S')] kernel=$(grep -ca 'elr=0xfffff8' $LOG) evtpyDUMP=$(grep -ca '\[evtpy\] === DUMP' $LOG)" | tee -a "$LOG"
  fi
done
echo "=== Path B watcher done $(date '+%H:%M:%S') ===" | tee -a "$LOG"
echo "--- [evtpy] lines ---" | tee -a "$LOG"
grep -a '\[evtpy\]' "$LOG" | tail -80 | tee -a "$LOG"
