#!/bin/bash
# S6-D16: lower advertised resolution to 720@60, RGB SDR; no device reboot.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-720p-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
try:
    ...
    link=TahoeLink(i,p,(3456,3456,4736,4736),os.environ['NWOAS_PROTOCOL_LOG'])
    link.sequence,link.command_sequence=21,20
    u=ProxyUtils(p)
