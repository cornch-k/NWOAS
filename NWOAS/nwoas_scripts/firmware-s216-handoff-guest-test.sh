#!/bin/bash
# S216: S192 native RTC/P12 and bounded HPD wait, with S193 host CPU init omitted.
...
PAYLOAD="$ROOT/nwoas_scripts/firmware-s216/m1n1-payload-s216-compat.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/firmware-s216-handoff-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = 0ae1cb75bfc3cbc2b7b15e6408c489fe5badd3d12d53fc4353a85b6b5ad3d197 ]
...
echo '[S216] S215 guarded guest placement; S216 reserved handoff copies; S208 gap50' | tee -a "$LOG"
