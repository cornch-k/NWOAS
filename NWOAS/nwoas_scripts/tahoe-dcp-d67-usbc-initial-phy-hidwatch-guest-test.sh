#!/bin/bash
# S6-D67: single initial USB-C PHY recovery after two seconds of ready host state.
# PASS: USB-C CCS/event and Windows device input. Build is HARDWARE-UNVERIFIED.
# FAIL: recovery without CCS/event; no retry loop. FL1100 D66 path retained.
...
HV="$M1N1/build/m1n1-d67-usbc-initial-phy.bin"
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-d67-usbc-initial-phy-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "32cd11528bdd5c1e73e99c31448afff511f354c708879fca4a203293e6e118a0" ]
...
echo "[DESIGN] D67 bounded initial USB-C PHY recovery; D66 FL1100 path retained" | tee -a "$LOG"
