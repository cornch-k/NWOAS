#!/bin/bash
# S215: S203 source-built compat prefix and original 256.875MiB header; diagnostic-only HV change.
...
PAYLOAD="$ROOT/nwoas_scripts/loader-s215/m1n1-payload-s215-original-fd.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/loader-s215-placement-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = bb930c722e19751eeb7a331680d396444ac4d1defd020fa35b041c226b738d83 ]
...
echo '[S215] S215 guarded guest placement, original S192 FD, S208 gap50' | tee -a "$LOG"
