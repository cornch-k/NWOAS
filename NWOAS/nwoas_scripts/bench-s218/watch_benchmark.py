...
while time.monotonic()<deadline:
 ...
 proc=subprocess.run([sys.executable,str(root/'transport-s123/queue_job.py'),str(link),str(root/'bench-s218/inspect.cmd')],capture_output=True,text=True)
 ...
 print('HOST_UTC',time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'JOB',job,done,flush=True);print('\n'.join(line for line in text.splitlines() if 'unique os id' not in line.lower()),flush=True)
 if done.get('exit') or 'NATIVE_EXIT=' in text or 'END_UTC=' in text or 'CB_TIMEOUT' in text:break
