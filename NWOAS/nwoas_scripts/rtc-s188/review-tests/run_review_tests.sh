#!/bin/sh
# S188 review tests (copy of the S185 runner). Compiles the S188 library
# sources from ../NwoasHardwareBootRtcLib/ (S185 + unseeded GetTime =>
# EFI_DEVICE_ERROR) against host shim headers and runs the integration test
# plain (-O2) and under ASan+UBSan. Also rebuilds and runs
...
SRC="$LIB/NwoasHardwareBootRtcLib.c $LIB/NwoasRtcSeedCore.c $LIB/nwoas_sera_rtc.c $HERE/test_s188_integration.c"

echo "== integration: plain -O2"
$CC $WARN -O2 $INC $SRC -o "$OUT/s188_plain"
"$OUT/s188_plain"

echo "== integration: ASan + UBSan -O1"
$CC $WARN -O1 -g -fno-omit-frame-pointer -fsanitize=address,undefined -fno-sanitize-recover=all \
  $INC $SRC -o "$OUT/s188_asan"
UBSAN_OPTIONS=print_stacktrace=1 ASAN_OPTIONS=detect_leaks=0 "$OUT/s188_asan"
...
echo "== S181 core suite against the S188 copy of NwoasRtcSeedCore.c (plain + ASan/UBSan)"
