#!/bin/bash
# S6-D82: D81 + bounded Link TRB traversal (8 segments,256 TRBs,cycle guard).
...
HV="$M1N1/build/m1n1-d82-usbc-link-walk.bin"
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-d82-usbc-link-walk-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "5abe7e3eebe08032a4c5f9fa9548cdc15cc1f6c3ed92f51f98d2d41f6fd3a131" ]
...
echo "[DESIGN] D82 USB1 low payload alias before endpoint doorbell; input hardware-unverified" | tee -a "$LOG"
