#!/bin/bash
# S6-D23: one public AV wakeDisplay call after verified standard-open success.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-av-wake-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
try:
    ...
    link=TahoeLink(i,p,(896,896,2048,2048),prefix,**{k:state[k] for k in ('boot_base','shared','syslog_base','asc_base','endpoint')},interface=5,half_size=0x2000)
    link.sequence,link.command_sequence=4,3
    msg=struct.pack('<2xHIII48x',8,10,16,0x69706378)+bytes(16)
    response=link.command(msg,96)
    assert len(response)>=64
    group,cmd,length,magic=struct.unpack_from('<2xHIII',response)
    assert (group,cmd,magic)==(8,10,0x69706378)
    print('[FACT] AV wakeDisplay status0')
