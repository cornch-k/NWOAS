#!/bin/bash
# S6-D80: USB1 DART translation of guest low-window buffers, before Windows RS.
# ONLY change from D79: ERST clean before Run / event invalidate before IRQ.
# PASS: mapping logs, no HSE, USB-C transfer completions + physical input.
# Safety: USB0/debug DART untouched; halted controller only; one chainload,
# caller-owned one bootstrap, 115200, no automatic retry; no disk writes.
set -eu
ROOT=/Volumes/X31/NWOAS
M1N1="$ROOT/m1n1_windows"
DEV=/dev/cu.usbmodemC07HL05SQ6NY1
PY="$M1N1/.venv-hv/bin/python3"
HV="$M1N1/build/m1n1-d80-usbc-event-coherence.bin"
PAYLOAD="$M1N1/m1n1-payload-iort-noleafdma-rering-runtime-dart-v1.bin"
HOOK="$M1N1/tahoe_dcp_guest_hook.py"
MODULE="$M1N1/pcie_emul-d77-usbc-dma.py"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-d80-usbc-event-coherence-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "1eb03f7883bd289abb89ffcef22fb1cd572dc5062cd287ba2f520a9ee5a3a0f6" ]
...
echo "[DESIGN] D80 USB1 DMA low-window translation before Windows Run; input hardware-unverified" | tee -a "$LOG"
