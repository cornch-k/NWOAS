#!/bin/bash
# S6-D25: ONLY variable vs D18 is omission of the AFK internal OPEN report.
# Hypothesis: internal OPEN changes firmware ownership before iBoot commands.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-no-open-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
try:
    ...
    link.sequence,link.command_sequence=0,0
    print('[DESIGN] No AFK internal OPEN; direct iBoot read-only query first')
