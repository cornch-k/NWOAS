#!/bin/bash
# S6-D48: D47's AC64=0/runtime-DART/common-bugcheck baseline plus one
# deferred ring snapshot after Windows advances ERDP beyond the event-ring base.
# ONLY VARIABLE from D47: observational POST-CONSUME event/command/DART dump.
# PASS: POST-CONSUME after real CTLR-POST, with the consumed type-34 event and
# any subsequent Enable Slot / Address Device command visible in the rings.
# WHEA capture remains BUGCHECK 0x124 plus BUGCHECK-SERR/BUGCHECK-VGIC state.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-postconsume-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D48 D47 baseline plus deferred post-consume event/command-ring snapshot" | tee -a "$LOG"
