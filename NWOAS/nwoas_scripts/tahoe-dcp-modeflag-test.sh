#!/bin/bash
# S6-D14: ONLY variable is the other advertised 1080@60 mode (trailing flag0 vs64).
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-modeflag-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
t=next(timings[4+j*24:28+j*24] for j in range(nt) if struct.unpack_from('<6I',timings,4+j*24)==(1,1920,1080,60<<16,0,0))
...
try:
    ...
    link=TahoeLink(i,p,(2432,2432,3840,3840),os.environ['NWOAS_PROTOCOL_LOG'])
    link.sequence,link.command_sequence=14,13
    u=ProxyUtils(p)
