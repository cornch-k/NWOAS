#!/bin/bash
# S167: bounded high RAM trial after S161 control; S163 50us HV, P7 unchanged.
...
PAYLOAD="$ROOT/nwoas_scripts/memory-s167/m1n1-payload-s167-8cpu-usba.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/memory-s167-download-p12-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = c253beb63eed818c319bdda57bc8e5a25c8042c31f55c23f10811ec758a12e4d ]
...
echo '[S167] 4 GiB high RAM plus low window; late P12, S163 gap50us; S168 delivery registered' | tee -a "$LOG"
