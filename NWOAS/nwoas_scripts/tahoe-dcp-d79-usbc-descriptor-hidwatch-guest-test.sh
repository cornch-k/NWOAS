#!/bin/bash
# S6-D79: USB1 DART translation of guest low-window buffers, before Windows RS.
# ONLY change from D78: bounded descriptor and late ring diagnostics.
...
HV="$M1N1/build/m1n1-d79-usbc-descriptor.bin"
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-d79-usbc-descriptor-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "9885cdc7761162f8554bf551df271bd636c21f19c41d9506630c951a5effbff8" ]
...
echo "[DESIGN] D79 USB1 DMA low-window translation before Windows Run; input hardware-unverified" | tee -a "$LOG"
