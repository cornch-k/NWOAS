#!/usr/bin/env python3
"""S210 inner builder: temporarily edit and exactly restore UEFI sources, build
...
generated/output paths to nwoas_scripts/firmware-s210, uses nice -n 19 / -j2,
...
OUT = ROOT / 'nwoas_scripts/firmware-s210'
VARIANT = os.environ.get('NWOAS_GUEST_VARIANT', 'compat')
assert VARIANT in ('compat', 'head')
PAY = 'm1n1-payload-s210-' + VARIANT + '.bin'
...
inner = inner.replace('OUT = ROOT / "nwoas_scripts/smp-s131"', 'OUT = ROOT / "nwoas_scripts/firmware-s210/s131-build"')
inner = inner.replace('target = ROOT / "m1n1_windows/m1n1-payload-s131-8cpu.bin"',
                      'target = ROOT / "nwoas_scripts/firmware-s210/' + PAY + '"')
...
try:
    ...
    if fdf_conflict:
        raise RuntimeError('Concurrent MacMini2020.fdf edit preserved; original not restored by S210')
    if conflicts:
