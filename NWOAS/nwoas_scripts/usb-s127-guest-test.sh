#!/bin/bash
# S127: S125 plus explicit BDS NVMe connection. PASS: UEFI I/O before EBS.
...
PAYLOAD="$M1N1/m1n1-payload-s127-uefinvme-connect.bin"
HOOK="$M1N1/tahoe_dcp_guest_hook.py"
MODULE="$M1N1/pcie_emul-d81-usbc-payload.py"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s127-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = "21a975e3961f7831146f72b96efb78873192654a00e774f9651a4f911d7babc0" ]
if /usr/sbin/lsof -t "$DEV" >/dev/null 2>&1; then
    echo "STOP: serial busy"
    exit 1
fi
shasum -a 256 "$HV" "$PAYLOAD" "$HOOK" "$MODULE" >> "$LOG"
echo "[S127] Transport bring-up: mini Windows zone write trial, second namespace in host RAM" | tee -a "$LOG"
