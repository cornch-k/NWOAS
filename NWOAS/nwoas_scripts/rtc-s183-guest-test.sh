...
PAYLOAD="$ROOT/nwoas_scripts/memory-s180/m1n1-payload-s180-10g-usba.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/rtc-s183-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
export NWOAS_RTC_SNAPSHOT=1 NWOAS_SMC_RTC_PROBE=1
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = 9d4664041dcf048a0ffb5688f2d157b1aff2cc8377633d9ce423a6a5846d7a65 ]
...
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/rtc-s178/probe_bounded.py" | awk '{print $1}')" = d983db99ed2f9f5bc59e469070bfbd2d77a01e9068fc91e05df40aa147beaa1e ]
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/rtc-s178/bounded_spmi.py" | awk '{print $1}')" = 6053c28ce4cea596514e383ace0cc8e244184d720f4dd208db2b450d92bbffcd ]
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/rtc-s178/rtc_math.py" | awk '{print $1}')" = ea69ea5a85a2e8cd95f0805c256733c8627ee76aedd5af1530ad71b272ea52f0 ]
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/rtc-s183/probe.py" | awk '{print $1}')" = 90f9789d07b0f9eb9d27cde39d36741c4441c8e66c0dab64c50c4948980a2bec ]
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/rtc-s183/bounded_smc.py" | awk '{print $1}')" = 8b89c76a398726ff983ea7ea6f70c1e057f1b4d85d413c4d3169be65daac8d6c ]
...
echo '[S180] first10GiB high RAM plus low window; S16350us, lateP12; no memory success claimed before hardware tests' | tee -a "$LOG"
...
"$PY" -u "$M1N1/proxyclient/tools/run_guest.py" -r --strict-init \
    ...
    -m "$ROOT/nwoas_scripts/rtc-s178/probe_bounded.py" \
    -m "$ROOT/nwoas_scripts/rtc-s183/probe.py" \
