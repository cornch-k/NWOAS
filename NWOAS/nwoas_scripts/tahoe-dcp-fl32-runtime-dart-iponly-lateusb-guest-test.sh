#!/bin/bash
# S6-D57: D56 runtime plus a one-shot late xHCI memory-state probe.
# ONLY VARIABLE from D56: after the +120s framebuffer mask, read command/event
# rings, DCBAA and live output device contexts.  No MMIO or guest-memory writes.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-iponly-lateusb-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D57 D56 runtime unchanged; add one-shot +120s read-only late USB ring/context probe" | tee -a "$LOG"
