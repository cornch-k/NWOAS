    ...
    def write(self,offset,value,width):
        ...
        if offset==0xc:self.mask|=value;self.update_irq()
            ...
            if (value>>14)&3 and not(self.csts&2) and (self.csts&12)!=8:
                self.csts=(self.csts&~12)|4 # shutdown processing
                try:
                    if hasattr(self.ns,'flush'):self.ns.flush()
                except (OSError,TimeoutError) as e:
                    return self.fatal('S122 shutdown flush failed: '+str(e))
                self.csts=(self.csts&~12)|8
                self.log('S122 SHUTDOWN durable flush complete')
