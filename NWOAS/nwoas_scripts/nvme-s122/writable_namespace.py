...
class WindowWritableNamespace(ReadOnlyNamespace):
    ...
    def _durable_success(self):
        # S122: VWC remains advertised absent, so every successful write must
        # reach nonvolatile storage before completion (including FUA writes).
        try:self.flush()
    ...
    def _io(self,command,mem=None):
        ...
        slba=cdw[0]|(cdw[1]<<32)
        nlb=(cdw[2]&0xffff)+1
        # S122: FUA is honored by flushing every write; LR/DSM remain hints.
        ...
        if not gpt_write and self._try_direct(True,slba,nlb,command):return self._durable_success()
        ...
        if gpt_write:
            result=self._gpt_write(slba,nlb,data)
            return self._durable_success() if result.status==SUCCESS else result
