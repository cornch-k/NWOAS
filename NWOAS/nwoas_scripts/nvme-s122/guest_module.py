# S103, executed after the D81 USB module and hv.load_raw().
...
sys.path.insert(0,'/Volumes/X31/NWOAS/nwoas_scripts/nvme-s122')
...
def backend_direct(write,lba,n,prp1,prp2):
    ...
    if not (1<=n<=16 and 0<=lba and lba+n<=61279344):return False
...
durability_flushes=0
def backend_flush():
    if not p.nvme_flush(1):raise OSError('ANS2 flush failed')
    global durability_flushes
    durability_flushes+=1
    if durability_flushes<=8 or durability_flushes%2000==0:hv.log(f'[S122] ANS FLUSH completed count={durability_flushes}')
...
c=Controller(WindowWritableNamespace(61279344,backend,backend_write,backend_flush,WINTEST_FIRST,WINTEST_LAST,gpt,backend_direct),memory,irq,log)
