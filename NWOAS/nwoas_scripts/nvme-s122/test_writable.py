...
class Test(unittest.TestCase):
 ...
 def test_gpt_guard_path_via_io(self):
  ...
  ents[2*128:3*128]=G.entry(G.DATA,FIRST,LAST)
  self.m.write(0x8000,bytes(ents[:4096]))
  self.assertEqual(self.st(1,q=1,ns=1,prp=0x8000,dw=(2,0,0)),0);self.assertEqual(d.writes,[]) # staged, not committed
  self.assertEqual(self.st(2,q=1,ns=1,prp=0xa000,dw=(2,0,0)),0);self.assertEqual(self.m.read(0xa000,4096),bytes(ents[:4096])) # read sees shadow
  bad=bytearray(d.b[1]);bad[40]^=1;self.m.write(0xc000,bytes(bad))
  self.assertEqual(self.st(1,q=1,ns=1,prp=0xc000,dw=(1,0,0)),0);self.assertEqual(d.writes,[]) # CRC-invalid header: staged, no commit yet
  self.m.write(0xc000,G.hdr_with(d.b[1],ents,1,61279343,2))
  self.assertEqual(self.st(1,q=1,ns=1,prp=0xc000,dw=(1,0,0)),0);self.assertEqual(sorted(d.writes),[1,2])
  self.m.write(0xd000,bytes(4096));self.assertEqual(self.st(1,q=1,ns=1,prp=0xd000,prp2=0xe000,dw=(5,0,1)),0x182) # 5..6 straddles table/data
  self.assertEqual(self.st(1,q=1,ns=1,prp=0xd000,dw=(6,0,0)),0x182) # LBA 6 (ISC) still refused
