#!/bin/bash
# S224: target-local read projection, S223 payload; existing admin owner retained.
# S208 diagnostic-corrected 50us NVMe control; host reads cluster state without initializing it.
...
HV="$ROOT/nwoas_scripts/native-s224/out/m1n1-s224-read-mirror.bin"
PAYLOAD="$ROOT/nwoas_scripts/loader-s223/m1n1-payload-s223-fd-tail.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/native-s224-read-mirror-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 6679506629bc77a3e428b33a5f4229009abdaef6b1e71087e4cebeaa6c222955 ]
...
[ "$(stat -f %z "$PAYLOAD")" -eq 33669120 ]
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = 2343f7fc35acd0401bafd63bfdc10d047ad8c4bc78207056c42e478051a8b5e7 ]
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/native-s224/query_module.py" | awk '{print $1}')" = fb299f2af1696c9ff473c6c89bded8de364e374634cbc1f83db77ad56de4fa26 ]
...
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/native-s224/module.py" | awk '{print $1}')" = 3f278e0e2ac9a1da95171daeb6f243f5c51ed9385e3e6d475e9713e12349ff69 ]
...
echo '[S224] Local read projection; S223 payload; host admin writes remain' | tee -a "$LOG"
