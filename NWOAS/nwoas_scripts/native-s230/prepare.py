...
R=Path('/Volumes/X31/NWOAS');S=R/'nwoas_scripts';H=S/'native-s230';W=R/'m1n1-admin-s230'
...
if W.exists():raise SystemExit('Refuse to replace existing S230 worktree')
...
a='    if (action == 0) {\n        nwoas_nvme_fp.armed = false;';assert t.count(a)==1;t=t.replace(a,'''    if(action==23)return 0x5332333000000001ULL;
    if(action==16)return s229_enable(sq_base,sq_size);
...
t=t.replace('    printf(\"NWOAS-S149 NVMe fastpath armed', '    if (!s229_enabled) printf(\"NWOAS-S149 NVMe fastpath armed')
p.write_text(t)
manifest={'stage':'S230','base_commit':base,'status':'Build/integration candidate, not hardware qualified','sources':{str(p.relative_to(W)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [W/'Makefile',W/'src/nvme.c',W/'src/hv_vm.c',W/'src/nwoas_s229_adapter.inc',*sorted(d.iterdir())]},'scope':'Local PCI/control/admin owner; existing ANS data path, NS2 and host boot setup retained'}
(H/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n');print('S230 worktree prepared')
