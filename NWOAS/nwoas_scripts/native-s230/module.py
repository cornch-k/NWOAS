...
if (_controller.cc or _controller.sq or _controller.cq or
    ...
    raise RuntimeError('S230 preboot controller/policy mismatch')
if p.nwoas_nvme_fastpath(23)!=0x5332333000000001:
    raise RuntimeError('S230 target capability mismatch')
if p.nwoas_nvme_fastpath(16,_link.block_count,_primary.mdts)!=1:
    raise RuntimeError('S230 target controller initialization failed')

def _unexpected_read(address,width):
    raise RuntimeError(f'S230 unexpected host NVMe read {address:#x}/{width}')

def _unexpected_write(address,value,width):
    raise RuntimeError(f'S230 unexpected host NVMe write {address:#x}/{width}')
...
hv.add_tracer(irange(0x700100000,0x4000),'s93-nvme-bar',TraceMode.HOOK,
              read=_unexpected_read,write=_unexpected_write)
hv.log('[S230] target owns PCI/control/admin; NS2 and host boot setup remain')
