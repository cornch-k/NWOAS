#!/bin/bash
# S6-D72: fix HPM ASCII index decoding in init/restore on D66 base.
# PASS: HPM1 powerup and physical USB-C input. No input evidence = unverified.
# Fresh bootstrap; verify automatic HPM powerup and USB-C register snapshot.
...
HV="$M1N1/build/m1n1-d72-usbc-gic-mask.bin"
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-d72-usbc-gic-mask-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "ceb5f2a1c97aed50c18578bafd77f4f42be2b1e3861859df2a5538da3c95eb90" ]
...
echo "[DESIGN] D72 gate USB-C SPI857 injection on guest GICD enable; D71 otherwise unchanged" | tee -a "$LOG"
