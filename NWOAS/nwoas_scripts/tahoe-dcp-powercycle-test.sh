#!/bin/bash
# S6-D13: ONLY variable is one display power-off/on cycle, no device reboot.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-powercycle-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
try:
    ...
    link=TahoeLink(i,p,(1920,1920,3456,3456),os.environ['NWOAS_PROTOCOL_LOG'])
    link.sequence,link.command_sequence=11,10
    u=ProxyUtils(p)
