#!/bin/bash
# S6-D71: fix HPM ASCII index decoding in init/restore on D66 base.
...
HV="$M1N1/build/m1n1-d71-i2c-enable-diagnostic.bin"
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-d71-i2c-enable-diagnostic-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "a5794ad9e2a95a9612729a149f8f51a31b53e182c84e886a70ece73e343e1634" ]
...
echo "[DESIGN] D71 I2C CTL.EN on rev>=6 plus bounded USB-C register snapshot" | tee -a "$LOG"
