...
def _fast_sync():
    global fast_armed
    if fast_armed:
        state=p.nwoas_nvme_fastpath(2,flags=_fast_flags())
        if state==2:
            fast_armed=False;c.fatal('S149 physical NVMe controller fault')
...
def mmio_write(addr,value,width):
    ...
    old_sq=c.sq.get(1);old_cq=c.cq.get(1)
    c.write(off,value,width)
    new_sq=c.sq.get(1);new_cq=c.cq.get(1)
    sq_changed=old_sq is not new_sq
    cq_changed=old_cq is not new_cq
    if sq_changed:fast_sq_generation=(fast_sq_generation+1)&0xfff
    if cq_changed:fast_cq_generation=(fast_cq_generation+1)&0xfff
    if fast_armed and (sq_changed or cq_changed or not _fast_ready()):_fast_disable()
    if not shutting_down:_fast_arm_if_ready()
    _fast_sync()
