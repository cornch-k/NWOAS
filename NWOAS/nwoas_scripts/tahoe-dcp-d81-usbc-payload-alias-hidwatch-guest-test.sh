#!/bin/bash
# S6-D81: D76 bypass baseline + low Normal/Data payload IPA-to-PA before DB.
...
HV="$M1N1/build/m1n1-d81-usbc-payload-alias.bin"
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-d81-usbc-payload-alias-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "949e6295d58a919ffabd067fc9339f26b807738314e8e96aab252bc88aa61b38" ]
...
echo "[DESIGN] D81 USB1 low payload alias before endpoint doorbell; input hardware-unverified" | tee -a "$LOG"
