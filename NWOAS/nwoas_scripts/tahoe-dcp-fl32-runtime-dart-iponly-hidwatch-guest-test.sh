#!/bin/bash
# S6-D59: D58 runtime plus a passive physical HID completion watcher.
# ONLY VARIABLE from D58: after the baseline probe, scan the controller-authored
# event ring once per second and log only a new slot-2 interrupt completion.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-iponly-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D59 D58 runtime unchanged; passively watch for real slot-2 HID interrupt completions" | tee -a "$LOG"
