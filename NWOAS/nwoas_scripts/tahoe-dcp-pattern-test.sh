#!/bin/bash
# S6-D10: output a 1080p60 RGB SDR color-bar pattern using known iBoot ops2/6/1.
# PASS stages: command replies; visible pattern still requires observation.
# Post-D9 state only. Zero reboots/reset/allocations/remapping. Back up VRAM.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-pattern-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
try:
    ...
    link=TahoeLink(i,p,(768,768,2688,2688),os.environ['NWOAS_PROTOCOL_LOG'])
    link.sequence,link.command_sequence=5,4
    u=ProxyUtils(p)
    ...
    (root/'experiments/tahoe-afk-20260906/d10-vram-before.bin').write_bytes(i.readmem(pa,size))
    palette=[(255,255,255),(255,255,0),(0,255,255),(0,255,0),(255,0,255),(255,0,0),(0,0,255),(32,32,32)]
    row=b''.join(bytes((b,g,r,255))*240 for r,g,b in palette)
    pattern=row*1080
    i.writemem(pa,pattern)
    p.dc_cvac(pa,size)
    service=TahoeIBoot(link)
    service.setPower(True)
    print('[FACT] Power command ACK')
    service.send_cmd(6,t+c)
    print('[FACT] 1920x1080 60Hz RGB SDR mode command ACK')
