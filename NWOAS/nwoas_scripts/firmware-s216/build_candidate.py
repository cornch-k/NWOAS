#!/usr/bin/env python3
"""S216: isolated UEFI Image-header correction candidate.
...
to nwoas_scripts/firmware-s216 so no S192 artifact or manifest is touched, and
...
out = R / 'nwoas_scripts/firmware-s216'
...
TRACKED = [
    "Silicon/Apple/T810XFamilyPkg/Library/MemoryInitPeiLib/MemoryInitPeiLib.inf",
...
try:
    ...
    m['change'] = ('S216 = S192 (conformant RTC unseeded error + ReadyToBoot P12, S180 '
    ...
    m['change'] = 'S216: S192 nativeRTC/P12 plus source-built S215 guest placement guard, 30MiB header, pre-copy FD overlap checks, and permanent BootArgs/ADT HOB reservation. Hardware status is recorded separately.'
