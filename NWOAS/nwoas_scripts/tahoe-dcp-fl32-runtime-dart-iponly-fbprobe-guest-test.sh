#!/bin/bash
# S6-D55: D51 runtime plus a one-shot EL2 framebuffer baseline comparison.
# ONLY VARIABLE from D51: observational full-surface hash/color-bar comparison at +120s.
# PASS: FBPROBE reports whether Windows changed the preserved scanout surface; no runtime writes.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-iponly-fbprobe-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D55 D51 runtime unchanged; one-shot EL2 framebuffer probe at kernel+120s" | tee -a "$LOG"
