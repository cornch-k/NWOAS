#!/bin/bash
# S6-D54: D51 runtime unchanged; capture the preserved 1280x720 BGRA framebuffer.
# ONLY VARIABLE from D51: observational automatic framebuffer read after 180 seconds.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-iponly-fbsnapshot-auto-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D54 D51 runtime unchanged; automatic 180-second framebuffer snapshot" | tee -a "$LOG"
...
exec "$PY" -u "$M1N1/proxyclient/tools/run_guest_fbsnapshot_auto.py" --fbout "$LOG.fb.bgra" --snapshot-after 180 -r \
