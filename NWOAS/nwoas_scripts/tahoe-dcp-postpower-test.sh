#!/bin/bash
# S6-D12: ONLY variable is power-on after modeset, then resubmit the same surface.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-postpower-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
try:
    ...
    link=TahoeLink(i,p,(1536,1536,3200,3200),os.environ['NWOAS_PROTOCOL_LOG'])
    link.sequence,link.command_sequence=9,8
    u=ProxyUtils(p)
    ...
    print('[FACT] Post-modeset power-on ACK')
