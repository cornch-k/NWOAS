#!/bin/bash
# S123 transport-only: SSD read-only, no automatic install. S103/S102 unchanged.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s123-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[S123] Transport bring-up: mini SSD read only, second namespace in host RAM" | tee -a "$LOG"
...
exec "$PY" -u "$M1N1/proxyclient/tools/run_guest.py" -r \
    --pre-init-script "$HOOK" -m "$MODULE" -m "$ROOT/nwoas_scripts/nvme-s93/guest_module.py" -m "$ROOT/nwoas_scripts/transport-s123/guest_module.py" "$PAYLOAD" >> "$LOG" 2>&1
