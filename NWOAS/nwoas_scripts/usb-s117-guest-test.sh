#!/bin/bash
# S117 unbuffered USB-to-NVMe copy using unchanged S103/S102 binaries.
# PASS requires both copied SWMs to match original SHA256 in both read modes.
# COPY117.CMD writes only a new directory after matching the preserved target file.
# Arm S117.TAG only; do not arm the old S100 reformat/install routine.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s117-$(date '+%Y%m%d-%H%M%S').XXXXXX")
