#!/bin/bash
# S230: target-local control/admin, S223 payload; existing data path and NS2 transport retained.
...
HV="$ROOT/nwoas_scripts/native-s230/out/m1n1-s230-admin.bin"
PAYLOAD="$ROOT/nwoas_scripts/loader-s223/m1n1-payload-s223-fd-tail.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/native-s230-admin-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 2d763bfb3aa6c55d8fbcd3d8ed90eeb4bb322f177d87ae279c1c53efbe7fa2c1 ]
...
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/native-s230/query_module.py" | awk '{print $1}')" = af4eeb7f45aa9669c264d2e3520ae3d79da4020c89354e85738989c55f80f09b ]
...
[ "$(shasum -a 256 "$ROOT/nwoas_scripts/native-s230/module.py" | awk '{print $1}')" = e3a415a88b8cee8721fca728780d111655f38ca7751d6ada6ad4eb0fff6aeb46 ]
...
echo '[S230] Local control/admin; S223 payload; host NS2 remains' | tee -a "$LOG"
...
"$PY" -u "$M1N1/proxyclient/tools/run_guest.py" -r --strict-init \
    ...
    -m "$ROOT/nwoas_scripts/native-s230/module.py" \
    -m "$ROOT/nwoas_scripts/native-s230/query_module.py" \
