#!/bin/bash
# S6-D45: D42's AC64=0 low-DMA allocation plus a UEFI payload whose live
...
# ONLY VARIABLE from D44: move the one-shot probe from the exported veneer to the common bugcheck entry.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-commonbug-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D45 D43 runtime-DART baseline plus common bugcheck-entry probe" | tee -a "$LOG"
