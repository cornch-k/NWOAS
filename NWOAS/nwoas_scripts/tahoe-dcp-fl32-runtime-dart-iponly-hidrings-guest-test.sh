#!/bin/bash
# S6-D58: D57 runtime plus bounded ring summaries for the configured HID slot.
# ONLY VARIABLE from D57: stop command parsing at its Link TRB; summarize all
# event slots and read the live HID transfer queues/report buffers. No MMIO or
# guest-memory writes.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-iponly-hidrings-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D58 D57 runtime unchanged; summarize event slots and live HID transfer/report rings" | tee -a "$LOG"
