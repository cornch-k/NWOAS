#!/bin/bash
# S144: S141 CPU/NVMe behavior with proxy and diagnostics on separate CDC pipes.
set -eu
...
HV="$M1N1/build/m1n1-s144-split-cdc.bin"
PAYLOAD="$M1N1/m1n1-payload-s139-8cpu-usba.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s144-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 251dd022803390f42c48b0e8d73d910604682463af8d200d0ae856eafcb09fd4 ]
...
echo '[S144] S141 CPU/NVMe + split CDC: NY1 proxy-only, NY3 diagnostics' | tee -a "$LOG"
