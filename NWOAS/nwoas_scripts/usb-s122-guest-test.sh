#!/bin/bash
# S122 apply verified local SWMs after guarded cleanup of failed trial install.
# PASS: DISM exit0 and expected kernel/loader/hive; first boot remains untested.
# S122.TAG only; S100 must remain disarmed. S103/S102 binaries unchanged.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s122-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
exec "$PY" -u "$M1N1/proxyclient/tools/run_guest.py" -r \
    --pre-init-script "$HOOK" -m "$MODULE" -m "$ROOT/nwoas_scripts/nvme-s122/guest_module.py" "$PAYLOAD" >> "$LOG" 2>&1
