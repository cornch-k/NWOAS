#!/bin/bash
# NWOAS EXP-5 5c v8 (XhciDxe instrumented) — capture the NWOAS-XHC wedge log to split the BCD-read
# transfer-completion failure 3 ways (halt / empty ring / unmatched). Auto-retry flaky boots.
set -u
LOG=/Volumes/X31/NWOAS/nwoas_scripts/logs/xhctest-v8.log
...
for attempt in 1 2 3 4 5; do
  ...
  M1N1DEVICE=/dev/cu.debug-console NWOAS_WIN_BASE=0 NWOAS_WIN_SIZE=0x100000000 M1N1_KEEP_BAUD=1 NWOAS_LOG="$LOG" \
    ...
    -r m1n1-payload-exp5c-v8-xhcidbg.bin -m pcie_emul-exp5.py >> "$LOG" 2>&1 &
  RGPID=$!
  ...
  for i in $(seq 1 300); do
    ...
    if [ "$(grep -ca 'NWOAS-XHC wedge' "$LOG")" -ge 3 ]; then
