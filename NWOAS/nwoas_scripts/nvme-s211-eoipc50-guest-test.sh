#!/bin/bash
# S211: S203 source-built compat prefix and original 256.875MiB header; diagnostic-only HV change.
# S208 architectural EOI PC diagnostic; host reads cluster state without initializing it.
...
PAYLOAD="$ROOT/nwoas_scripts/loader-s197/out/compat/m1n1-payload-s197-compat.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/nvme-s211-eoipc50-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = 9a0b96af3e09cbd4450590ac4ff59e5e2aa4949bd70723ad9b8240996635020a ]
...
echo '[S211] S197 compat prefix; S208 architectural EOI PC diagnostic, gap50' | tee -a "$LOG"
