#!/bin/bash
# S204: S203 compat with source-built 30MiB UEFI image_size header.
...
PAYLOAD="$ROOT/nwoas_scripts/firmware-s204/m1n1-payload-s204-imghdr-compat.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/loader-s204-imghdr-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = 548fd6b035e58024fc61969ae2ad14ce5efa5b84a18a503c35a20ba7691bd783 ]
...
echo '[S204] S197 compat prefix; only UEFI image_size corrected256.875MiB to30MiB' | tee -a "$LOG"
