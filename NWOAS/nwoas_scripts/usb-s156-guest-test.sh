#!/bin/bash
# S156: S154 plus ERDP-selected FL1100 event coherence.
set -eu
...
HV="$M1N1/build/m1n1-s156-fl1100-erdp-segment.bin"
PAYLOAD="$M1N1/m1n1-payload-s135-8cpu-speed.bin"
USBC="$M1N1/pcie_emul-d81-usbc-payload.py"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s156-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = e8a81a9f744a3a5f540bf8424e2342417db9e558a0ff60c8e0c517bd5c720748 ]
...
echo '[S156] S154 + ERDP-selected FL1100 event invalidate on EINT/IP' | tee -a "$LOG"
