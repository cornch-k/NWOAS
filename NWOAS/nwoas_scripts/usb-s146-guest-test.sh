#!/bin/bash
# S146: S145 filtered split CDC with the proven 64 KiB NVMe MDTS restored.
set -eu
...
HV="$M1N1/build/m1n1-s146-filtered-cdc-64k.bin"
PAYLOAD="$M1N1/m1n1-payload-s139-8cpu-usba.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s146-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = 6fc2d42f5fc33c72b4e4da6ed32c0ce6526425f68490d6c5fb7b8039b7ab61ca ]
...
echo '[S146] S140 64KiB NVMe + split CDC: NY1 proxy-only, NY3 filtered diagnostics' | tee -a "$LOG"
