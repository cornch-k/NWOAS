...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/memory-s161-upload-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
export NWOAS_BACKUP_INSTALL_SOURCE=1
...
echo '[S161] normalized RAM/backing, S163 gap50us, 8 cores USB-A, unchanged Windows cap' | tee -a "$LOG"
