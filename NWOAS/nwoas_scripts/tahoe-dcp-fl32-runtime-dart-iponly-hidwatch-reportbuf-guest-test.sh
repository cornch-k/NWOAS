#!/bin/bash
# S6-D62: D61 watcher with bounded post-input HID report-buffer evidence.
# ONLY VARIABLE from D61: retain the bounded interrupt-IN report buffers
# found during late enumeration and dump up to 32 bytes after first completion.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-iponly-hidwatch-reportbuf-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D62 D61 runtime unchanged; dump bounded saved HID report buffers after first completion" | tee -a "$LOG"
