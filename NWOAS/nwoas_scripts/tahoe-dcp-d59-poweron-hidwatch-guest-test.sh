#!/bin/bash
# S6-D63: reproduce the hardware-proven D59 runtime with the HDMI sink powered on.
# ONLY VARIABLE from successful D59: physical monitor power/observation state.
# The exact preserved D59 outer binary is used instead of mutable build/m1n1.bin.
...
HV="$M1N1/build/m1n1-d59-hidwatch.bin"
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-d59-poweron-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "6bb61f7a6367073397790fe1f1f668f526d9847652ae76629f0adca1b5b03277" ]
...
echo "[DESIGN] D63 exact D59 runtime; only monitor is powered on for physical scanout/HID observation" | tee -a "$LOG"
