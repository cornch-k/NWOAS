#!/bin/bash
# S6-D52: D51 runtime unchanged; capture the preserved 1280x720 BGRA framebuffer.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-iponly-fbsnapshot-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D52 D51 runtime unchanged; bounded post-run framebuffer snapshot" | tee -a "$LOG"
