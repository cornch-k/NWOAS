#!/bin/bash
# S163: compare 50us NVMe EOI spacing against 200us; same S139 8-core payload.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s163g50-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo '[S163] IRQ900 EOI reassert gap50us, S162 CQ probe, 8 cores USB-A' | tee -a "$LOG"
