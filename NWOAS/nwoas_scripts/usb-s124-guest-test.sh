...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s124-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[S124] Transport bring-up: mini Windows zone write trial, second namespace in host RAM" | tee -a "$LOG"
...
exec "$PY" -u "$M1N1/proxyclient/tools/run_guest.py" -r \
    --pre-init-script "$HOOK" -m "$MODULE" -m "$ROOT/nwoas_scripts/nvme-s124/guest_module.py" -m "$ROOT/nwoas_scripts/nvme-s124/link_module.py" "$PAYLOAD" >> "$LOG" 2>&1
