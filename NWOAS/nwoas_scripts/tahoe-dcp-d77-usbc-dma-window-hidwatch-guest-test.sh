#!/bin/bash
# S6-D77: USB1 DART translation of guest low-window buffers, before Windows RS.
# ONLY behavioral change: use validated FL1100 runtime page tables for USB1.
...
HV="$M1N1/build/m1n1-d77-usbc-dma-window.bin"
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-d77-usbc-dma-window-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "1f9ff8a7bf90e7e261ff824213f937d15be436b2e66fa858f5208e849cd29dae" ]
...
echo "[DESIGN] D77 USB1 DMA low-window translation before Windows Run; input hardware-unverified" | tee -a "$LOG"
