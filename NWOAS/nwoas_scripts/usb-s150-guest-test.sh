#!/bin/bash
# S150: target-owned NVMe queue with dynamic namespace-2 host RPC.
set -eu
...
HV="$M1N1/build/m1n1-s150-target-nvme-link.bin"
PAYLOAD="$M1N1/m1n1-payload-s139-8cpu-usba.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s150-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = d0c5e2eb5c94a4dba302c3d6e6571ff64706b9d19db23c69a3d803562761a546 ]
...
echo '[S150] target-owned NVMe I/O queue; NS1 direct, NS2 data-only host RPC' | tee -a "$LOG"
