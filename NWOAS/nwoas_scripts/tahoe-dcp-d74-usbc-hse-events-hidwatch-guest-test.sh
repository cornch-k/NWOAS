#!/bin/bash
# S6-D74: fix HPM ASCII index decoding in init/restore on D66 base.
...
HV="$M1N1/build/m1n1-d74-usbc-hse-events.bin"
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-d74-usbc-hse-events-hidwatch-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "b837648fead9247daccf9a5c031fff648c29d347930ee89aa8132cbb8111204f" ]
...
echo "[DESIGN] D74 one HSE snapshot of USB-C event ring and DART fault state on D72" | tee -a "$LOG"
