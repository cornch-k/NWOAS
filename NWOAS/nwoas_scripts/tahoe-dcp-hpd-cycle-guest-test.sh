#!/bin/bash
# S6-D38: ONLY display variable versus D37 is transition-based settling.
# Wait for Tahoe's actual HPD True->False->True cycle, then apply 720p and the
# color-bar surface once to the republished interface. BootArgs and USB seed
# diagnostics remain identical to D37.
# PASS (display): color bars remain during the serviced five-second hold.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-hpd-cycle-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D38 wait for actual HPD down/up, then reapply once" | tee -a "$LOG"
