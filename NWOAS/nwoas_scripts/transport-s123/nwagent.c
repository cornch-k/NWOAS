/* S123 explicit WinPE foreground worker. Commands arrive only on the host RAM
...
static const U8 magic[16]="NWOAS-S123-LINK!";
...
static int run(U32 job,U32 n){
 static W script[]=L"X:\\NWOAS123.CMD",app[]=L"X:\\Windows\\System32\\cmd.exe";
 static W command[]=L"cmd.exe /d /c X:\\NWOAS123.CMD";
 H f=(H)-1,r=0,w=0;U32 got,available,rc=2;SA sa={sizeof(SA),0,1};SI si;PI pi;
 ...
 if(!CreatePipe(&r,&w,&sa,16384)||!SetHandleInformation(r,1,0))return 0;
 zero(&si,sizeof(si));zero(&pi,sizeof(pi));si.cb=sizeof(si);si.flags=0x100;
 si.input=GetStdHandle((U32)-10);si.output=w;si.error=w;
 if(!CreateProcessW(app,command,0,0,1,0,0,L"X:\\",&si,&pi)){
  rc=GetLastError();CloseHandle(r);CloseHandle(w);return send(job,3,(const U8 *)"CreateProcess failed",20,rc);
 }
 CloseHandle(w);CloseHandle(pi.thread);
...
void mainCRTStartup(void){
 ...
 emit("NWOAS S123 host link; foreground WinPE worker\r\n");
 if(GetEnvironmentVariableW(L"SystemDrive",env,8)!=2||env[0]!='X'||env[1]!=':'){emit("STOP: requires WinPE X:\r\n");ExitProcess(2);}
 ...
 sequence=*(U32 *)(buf+48);
 if(!send(0,4,(const U8 *)"WinPE worker connected",22,0))goto fail;
