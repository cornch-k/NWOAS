#!/usr/bin/env python3
"""S105: read-only. Parse the NTFS volume in the Windows partition (LBA 53920000, 4096-byte
...
import sys,struct,hashlib,signal,subprocess,time
NAME=sys.argv[1] if len(sys.argv)>1 else 't1.swm'
PART=53920000;BS=4096
...
p.nvme_shutdown()
print(f'SHA256 {name} = {h.hexdigest()} ({size} bytes, {time.monotonic()-t0:.0f}s)')
