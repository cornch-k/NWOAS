    ...
    for i in range(len(mft_bytes)//rec):
        ...
        while off+8<=len(r_):
            ...
            if typ==0x80 and not nl:
                if nr:
                    ro=struct.unpack_from('<H',r_,off+32)[0]
                    attrs.append(dict(size=struct.unpack_from('<Q',r_,off+48)[0],runs=runlist(r_[off+ro:off+length])))
                else:
            ...
            off+=length
        if any(x['name'].lower() in ('s120-report.txt','s120-apply-started.txt','s120-apply-pass.txt') for x in names):
            rows.append(dict(record=i,names=names,data=attrs))
    for row in rows:
        if not any(n['name'].lower()=='s120-report.txt' for n in row['names']):continue
        assert len(row['data'])==1
        a=row['data'][0];assert a['size']<=4*1024*1024
        if 'runs' in a:
            parts=[];left=a['size']
            for lcn,count in a['runs']:
                ...
                lba=PART+lcn*spc;n=count*spc
                while n and left:
                    k=min(n,1024,(left+BS-1)//BS);part=rd(lba,k)[:left];parts.append(part);left-=len(part);lba+=k;n-=k
            assert not left
            content=b''.join(parts)
        else:content=a['text'].encode('ascii')
        Path('nwoas_scripts/logs/s120-ssd-report.txt').write_bytes(content)
        print('SSD REPORT:',content.decode('cp949',errors='replace'),flush=True)
    report=dict(storage_writes=False,mft_bytes_scanned=len(mft_bytes),matches=rows)
    print(json.dumps(report,indent=2),flush=True)
    Path('nwoas_scripts/logs/s120-report-mft.json').write_text(json.dumps(report,indent=2)+'\n')
finally:
    try:
