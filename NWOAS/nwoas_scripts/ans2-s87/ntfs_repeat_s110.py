...
try:
    ...
    prior=json.loads(Path('/tmp/nwoas-s109-full-differences.json').read_text());items=prior['differences']
    assert prior['bytes']==size and len(runs)==1 and spc==1 and len(items)<2048
    output=Path('/tmp/nwoas-s110-blocks');output.mkdir(exist_ok=True)
    original=Path('/Volumes/WINARM2/sources/install.swm').open('rb');results=[]
    for item in items:
        assert item['lba']==PART+runs[0][0]+item['offset']//BS
        raw=rd(item['lba'],1);original.seek(item['offset']);expected=original.read(BS)
        stable=hashlib.sha256(raw).hexdigest()==item['actual_sha256']
        changed=[j for j,(aa,bb) in enumerate(zip(raw,expected)) if aa!=bb]
        report=dict(offset=item['offset'],lba=item['lba'],stable=stable,zero=not any(raw),changed_bytes=len(changed),first_changed=min(changed) if changed else None,last_changed=max(changed) if changed else None)
        results.append(report)
        (output/(str(item['lba'])+'.bin')).write_bytes(raw)
    original.close()
    report=dict(experiment='S110 repeated bad blocks',count=len(results),all_stable=all(x['stable'] for x in results),results=results,storage_writes=False)
    Path('/tmp/nwoas-s110-repeat.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report),flush=True)
finally:
    try:
