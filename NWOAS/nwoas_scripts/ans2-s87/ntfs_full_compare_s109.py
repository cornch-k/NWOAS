...
try:
    ...
    original=source.open('rb');offset=0;differences=[];t0=time.monotonic();digest=hashlib.sha256()
    for lcn,ln in runs:
        assert lcn is not None
        lba=PART+lcn*spc;n=ln*spc
        while n and offset<size:
            k=min(n,16);got=rd(lba,k);want=original.read(min(k*BS,size-offset));got=got[:len(want)];digest.update(got)
            if got!=want:
                for j in range(0,len(want),BS):
                    actual=got[j:j+BS];expected=want[j:j+BS]
                    if actual!=expected:
                        item=dict(offset=offset+j,lba=lba+j//BS,zero=not any(actual),actual_sha256=hashlib.sha256(actual).hexdigest(),expected_sha256=hashlib.sha256(expected).hexdigest())
                        differences.append(item);print('DIFFERENCE',json.dumps(item),flush=True)
            offset+=len(want);lba+=k;n-=k
            if offset%(256*1024*1024)<k*BS:print(f'COMPARED {offset} bytes {time.monotonic()-t0:.0f}s',flush=True)
    assert offset==size
    original.close()
    report=dict(experiment='S109 complete block comparison',bytes=offset,sha256=digest.hexdigest(),differences=differences,storage_writes=False,elapsed_seconds=time.monotonic()-t0)
    Path('/tmp/nwoas-s109-full-differences.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report),flush=True)
finally:
    try:
