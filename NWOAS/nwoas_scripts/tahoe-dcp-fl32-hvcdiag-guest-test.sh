#!/bin/bash
# S6-D42: retain D41's single-variable FL1100 HCCPARAMS1.AC64=0 experiment
# and add an observational dump when an inner-m1n1 HVC #0 reboot is trapped.
# PASS: FL32 marker plus kernel DCBAAP/ERSTBA/CRCR below 4GB, followed by real
# event consumption/ERDP progress. If pre-UEFI failure recurs, the captured
# UART dump must expose its original PC/ESR/FAR instead of losing it at HVC #0.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-hvcdiag-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D42 D41 AC64=0 retained; HVC#0 UART/PC-ring diagnostic added" | tee -a "$LOG"
