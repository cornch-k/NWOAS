#!/bin/bash
# S132: S131 8-core MADT plus PSCI CPU_OFF/CPU_ON lifecycle repair.
# S130 bounded NVMe logging and all S124/S126 safety guards are retained.
set -eu
ROOT=/Volumes/X31/NWOAS
M1N1="$ROOT/m1n1_windows"
DEV=/dev/cu.usbmodemC07HL05SQ6NY1
PY="$M1N1/.venv-hv/bin/python3"
HV="$M1N1/build/m1n1-s132-psci-lifecycle.bin"
PAYLOAD="$M1N1/m1n1-payload-s131-8cpu.bin"
HOOK="$M1N1/tahoe_dcp_guest_hook.py"
MODULE="$M1N1/pcie_emul-d81-usbc-payload.py"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s132-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = "19fd219568a06c23cc6c73f08288a970d052e82ecc76d51f876c6eeeb332833c" ]
...
echo "[S132] 8-core MADT + PSCI AP park/restart lifecycle; S129 SSD-first; S130 quiet NVMe" | tee -a "$LOG"
...
exec "$PY" -u "$M1N1/proxyclient/tools/run_guest.py" -r \
    --pre-init-script "$HOOK" -m "$MODULE" -m "$ROOT/nwoas_scripts/nvme-s130/guest_module.py" -m "$ROOT/nwoas_scripts/uefi-s125/link_module.py" "$PAYLOAD" >> "$LOG" 2>&1
