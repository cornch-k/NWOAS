#!/bin/bash
# S6-D49: D48 baseline plus a bounded dump of the WHEA CPER record supplied
# as bugcheck parameter 2.
# ONLY VARIABLE from D48: observational WHEA header/descriptor/raw record dump.
# PASS: BUGCHECK 0x124/0x12 with WHEA-CPER signature, section descriptors, and
# the ARM error section bytes preserved for offline decoding.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/tahoe-dcp-fl32-runtime-dart-whea-cper-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
echo "[DESIGN] D49 D48 baseline plus bounded WHEA CPER record dump at common bugcheck" | tee -a "$LOG"
