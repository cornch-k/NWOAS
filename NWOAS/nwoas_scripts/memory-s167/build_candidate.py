#!/usr/bin/env python3
"""Build isolated S167 4-GiB high-RAM output; temporarily edit and exactly restore UEFI sources.
...
OUT=ROOT/'nwoas_scripts/memory-s167'
...
inner=inner.replace('OUT = ROOT / "nwoas_scripts/smp-s131"','OUT = ROOT / "nwoas_scripts/memory-s167/s131-build"')
inner=inner.replace('target = ROOT / "m1n1_windows/m1n1-payload-s131-8cpu.bin"',
                    'target = ROOT / "nwoas_scripts/memory-s167/m1n1-payload-s167-8cpu-usba.bin"')
...
try:
    ...
    wrapper.write_text('#define NWOAS_HIDE_KEEP_HIGH_BYTES 0x100000000ULL\n'+(OUT/'NwoasHideHighRamDxe-s167.c').read_text())
    ...
    payload=OUT/'m1n1-payload-s167-8cpu-usba.bin'
    manifest={'payload':str(payload),'bytes':payload.stat().st_size,
              'sha256':hashlib.sha256(payload.read_bytes()).hexdigest(),
              'change':'S161 normalization plus first 4 GiB high RAM kept; 8 CPUs; XHC1 hidden',
