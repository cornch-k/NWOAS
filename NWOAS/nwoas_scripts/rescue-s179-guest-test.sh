...
PAYLOAD="$ROOT/nwoas_scripts/rescue-s179/m1n1-payload-s179-usb-first.bin"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/rescue-s179-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$PAYLOAD" | awk '{print $1}')" = adea72ef09592aa55b74e5b9bb1f748500469c61c5744b62027baaa37bb93ebf ]
...
echo '[S179] USB-first WinPE recovery; existing Windows partition preserved; no install job armed' | tee -a "$LOG"
