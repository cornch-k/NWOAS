#!/bin/bash
# S157: S154 baseline with no persisted USB-C transfer-ring cursor.
set -eu
...
HV="$M1N1/build/m1n1-s157-usbc-no-cached-segment.bin"
PAYLOAD="$M1N1/m1n1-payload-s135-8cpu-speed.bin"
USBC="$M1N1/pcie_emul-d81-usbc-payload.py"
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/usb-s157-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
[ "$(shasum -a 256 "$HV" | awk '{print $1}')" = d9eba5645b34ced6d43b8a72cfeba57c2c3f37ec9c2637683d11a5d1e28d14de ]
...
echo '[S157] S154 baseline + no persisted USB-C transfer-ring cursor' | tee -a "$LOG"
