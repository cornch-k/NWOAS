#!/bin/bash
# S164: apply P12 once after Windows worker connection, same S139/S163 control.
...
LOG=$(mktemp "$ROOT/nwoas_scripts/logs/cpufreq-s164-latep12-$(date '+%Y%m%d-%H%M%S').XXXXXX")
...
export NWOAS_CPU_PSTATE=12 NWOAS_LATE_P12=1
...
echo '[S164] late P12 once Windows connects; S163 gap50us, 8 cores USB-A' | tee -a "$LOG"
